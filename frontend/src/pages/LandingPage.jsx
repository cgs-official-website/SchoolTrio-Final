      ...
      try {
        ...
        if (isMounted && response?.data) {
          ...
          const sortedPlans = [...rawPlans].sort((a, b) => {
            const indexA = order.indexOf(a.id || a.key);
            const indexB = order.indexOf(b.id || b.key);
            if (indexA !== -1 && indexB !== -1) return indexA - indexB;
            if (indexA !== -1) return -1;
            if (indexB !== -1) return 1;
            return 0;
          });
          const visiblePlans = sortedPlans.filter(plan => plan.active !== false && plan.isActive !== false);
