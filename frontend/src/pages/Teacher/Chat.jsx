...
export default function TeacherChat() {
  ...
  const [parentLinked, setParentLinked] = useState(null);
  ...
  // Fetch students and rooms via REST API
  const fetchStudentsAndChats = useCallback(async () => {
    if (!schoolId) {
    ...
    try {
      // 1. Fetch class students if classId exists
      let classStudents = [];
      if (classId) {
        try {
          const studentsRes = await studentsApi.listStudents({ classId, limit: 100 });
          classStudents = Array.isArray(studentsRes?.data) ? studentsRes.data : [];
        } catch (err) {
          console.error("Error fetching class students:", err);
        }
      }

      // 2. Fetch teacher's chat rooms
      let rooms = [];
      try {
        const roomsRes = await chatsApi.listChatRooms({ limit: 100 });
      ...
      // Enrich with chat room metadata and add any out-of-class chat students
      rooms.forEach(room => {
        const existing = allStudentsMap.get(room.studentId);
        if (existing) {
          allStudentsMap.set(room.studentId, {
            ...existing,
            unreadCount: room.unreadCountTeacher || room.unreadCount_teacher || 0,
            lastMessageTime: room.lastMessageTime || null,
            roomId: room.id,
            parentName: room.parentName || null,
            parentId: room.parentId || null,
            status: room.status || 'active'
          });
        } else {
          // Out-of-class student with active chat
          const fullName = room.studentName || 'Student';
      ...
      const enrichedStudents = Array.from(allStudentsMap.values());
  ...
  const fetchChannels = useCallback(async () => {
    if (schoolId) {
      ...
      try {
        const res = await chatsApi.listChatChannels({ limit: 50 });
  ...
  // Handle Channel Selection & Posts Fetch
  useEffect(() => {
    let isCancelled = false;
    if (activeTab === 'channels' && activeChannel && schoolId) {
      setMessages([]);
      chatsApi.listChannelPosts(activeChannel.id, { limit: 100 })
        .then(res => {
          if (!isCancelled) {
  ...
  // Handle Student Selection & Room Resolution & Messages Fetch
  useEffect(() => {
    let isCancelled = false;
    ...
    const setupChat = async () => {
      if (activeStudent && schoolId) {
        ...
        try {
          // 1. Resolve or get canonical chat room
          const roomRes = await chatsApi.resolveChatRoom({ studentId: activeStudent.id });
          const room = roomRes?.data || roomRes;
          
          if (isCancelled) return;

          setChatRoomData(room);
          setParentName(room.parentName || (activeStudent.parentName || 'Parent'));
          setLinkedParentId(room.parentId || activeStudent.parentId || null);
          setParentLinked(Boolean(room.parentId || activeStudent.parentId));

          // 2. Fetch messages for the room
          const messagesRes = await chatsApi.listChatMessages(room.id, { limit: 100 });
          if (!isCancelled) {
            setMessages(Array.isArray(messagesRes?.data) ? messagesRes.data : []);
          }

          // 3. Mark chat as read if unread count > 0
          if (room.unreadCountTeacher > 0 || activeStudent.unreadCount > 0) {
            try {
              await chatsApi.markChatRoomRead(room.id);
              if (!isCancelled) {
                setStudents(prev => prev.map(s => s.id === activeStudent.id ? { ...s, unreadCount: 0 } : s));
                setChatRoomData(prev => prev ? { ...prev, unreadCountTeacher: 0, unreadCount_teacher: 0 } : prev);
              }
            } catch (err) {
  ...
  const handleSendMessage = async (text, mediaUrl, mediaType) => {
    ...
    try {
      if (activeTab === 'channels' && activeChannel) {
        ...
        const res = await chatsApi.createChannelPost(activeChannel.id, payload);
        const createdPost = res?.data || res;
        setMessages(prev => [...prev, createdPost]);
      } else if (activeTab === 'dms' && activeStudent) {
        let roomId = chatRoomData?.id;
        if (!roomId) {
          const roomRes = await chatsApi.resolveChatRoom({ studentId: activeStudent.id });
          const room = roomRes?.data || roomRes;
          roomId = room.id;
        ...
        const res = await chatsApi.sendChatMessage(roomId, payload);
        const createdMsg = res?.data || res;

        setMessages(prev => [...prev, createdMsg]);
        
        // Update local room metadata & student ordering
        const previewText = text || (mediaType ? `[${mediaType}]` : '[Attachment]');
        const now = createdMsg.createdAt || new Date().toISOString();

        setChatRoomData(prev => prev ? { ...prev, lastMessage: previewText, lastMessageTime: now } : prev);
        setStudents(prev => {
          const updated = prev.map(s => s.id === activeStudent.id ? { ...s, lastMessageTime: now } : s);
          updated.sort((a, b) => {
            if (a.lastMessageTime && b.lastMessageTime) {
              return new Date(b.lastMessageTime) - new Date(a.lastMessageTime);
            }
  ...
  const handleDeleteAction = async (_type) => {
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const roomsMap = new Map();
      rooms.forEach(c => roomsMap.set(c.studentId, c));

      // Construct rows based on roster
      const data = students.map(student => {
        const chatInfo = roomsMap.get(student.id);
        return {
          "Student ID": student.id,
          "Student Name": `${student.firstName} ${student.lastName || ''}`.trim(),
          "Parent Name": chatInfo?.parentName || student.parentName || 'Unknown',
                ...
                students.map(student => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${
                      ...
                      {(student.firstName || 'S').charAt(0)}{(student.lastName || '').charAt(0)}
                    </div>
        ...
        <div className={`flex-1 bg-white dark:bg-slate-900 md:rounded-3xl md:border border-slate-200 dark:border-slate-700 md:shadow-sm flex-col min-h-0 overflow-hidden relative ${
          ...
          {activeTab === 'dms' && activeStudent && (
            <>
              ...
              <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-12 h-12 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center font-bold text-lg">
                    {(activeStudent.firstName || 'S').charAt(0)}{(activeStudent.lastName || '').charAt(0)}
                  </div>
                  <div>
                ...
                {chatRoomData && (
                  <button
                    onClick={() => {
                      ...
                      setChatRoomData(prev => ({ ...prev, status: newStatus }));
                ...
                {loadingMessages ? (
                  ...
                  (() => {
                    ...
                    return messages.map(msg => {
                      const isMe = msg.senderRole === 'teacher' || msg.senderId === currentUserId;
                          ...
                          <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} group items-center gap-2`}>
                            ...
                            <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl p-4 ${
                              ...
                              <span className={`text-[10px] mt-1 flex items-center gap-1 ${isMe ? 'text-primary-200 justify-end' : 'text-slate-400 dark:text-slate-300 justify-start'}`}>
                                ...
                                {isMe && (
                                  <span title="Delivered" className="font-bold ml-1 tracking-tighter">
                                    ✓✓
                                  </span>
                                )}
                              </span>
              ...
              <ChatInput 
                schoolId={schoolId} 
                chatRoomId={chatRoomData?.id || activeStudent.id} 
              ...
              {/* Channel Header */}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        {activeChannel.className ? `Class ${activeChannel.className}` : 'Group Channel'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar bg-slate-50/30 dark:bg-slate-800/30">
                  {messages.length === 0 ? (
                    ...
                    messages.map((msg) => {
                      const isTeacher = msg.senderRole === 'teacher' || msg.senderId === currentUserId;
                      return (
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl p-6 relative">
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                const targetClassId = targetClass === 'all' ? null : (targetClass || classId);
                const res = await chatsApi.createChatChannel({
                  name: name.trim(),
                  classId: targetClassId
                });
                const newChannel = res?.data || res;
