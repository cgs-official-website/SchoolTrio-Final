...
export default function Grades() {
  ...
  const [initialGrades, setInitialGrades] = useState({}); // Baseline for clear-grade delta detection
  const [savingGrades, setSavingGrades] = useState(false);
  const [loadingGrades, setLoadingGrades] = useState(false);
  const [successMsg, setSuccessMsg] = useState('');
  const selectedAssessmentIdRef = useRef(null);
  ...
  const fetchAssessments = async (targetClassId) => {
    try {
      ...
      if (mountedRef.current && currentClassIdRef.current === targetClassId) {
        setAssessments(items);
        setActiveAssessment(prev => {
          if (!prev) return null;
          const matching = items.find(a => a.id === prev.id);
  ...
  useEffect(() => {
    ...
    const fetchStudentsData = async (targetClassId) => {
      try {
        const res = await listStudents({ classId: targetClassId, limit: 100 });
        if (!mountedRef.current || currentClassIdRef.current !== targetClassId) return;
        const studentsData = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
        const sorted = [...studentsData].sort((a, b) => (a.firstName || '').localeCompare(b.firstName || ''));
        setStudents(sorted);
      } catch (err) {
        console.error('Error fetching students in Grades:', err);
        if (mountedRef.current && currentClassIdRef.current === targetClassId) {
          setStudents([]);
        }
      }
  ...
  const handleSelectAssessment = async (assessment, studentList = students) => {
    if (!assessment) return;
    setActiveAssessment(assessment);
    selectedAssessmentIdRef.current = assessment.id;
    setSuccessMsg('');

    // Pre-populate empty entries for all students while loading
    const defaultMap = {};
    studentList.forEach(student => {
      defaultMap[student.id] = '';
    });
    setGrades(defaultMap);
    setInitialGrades(defaultMap);

    if (!assessment.id) return;

    setLoadingGrades(true);
    try {
      const res = await getAssessmentGrades(assessment.id);
      
      // Guard against stale out-of-order responses
      if (selectedAssessmentIdRef.current !== assessment.id) {
        return;
      }

      const gradesList = res?.data || [];
      const loadedGrades = { ...defaultMap };
      
      gradesList.forEach(item => {
        if (item.studentId && item.marksObtained !== undefined && item.marksObtained !== null) {
          loadedGrades[item.studentId] = Number(item.marksObtained);
        }
      });

      setGrades(loadedGrades);
      setInitialGrades({ ...loadedGrades });
    } catch (error) {
      if (selectedAssessmentIdRef.current === assessment.id) {
        console.error("Error fetching assessment grades:", error);
        toast.error("Failed to load assessment grades.");
      }
    } finally {
      if (selectedAssessmentIdRef.current === assessment.id) {
  ...
  const handleSaveGrades = async () => {
    if (!activeAssessment || savingGrades) return;
    ...
    try {
      ...
      students.forEach(student => {
        ...
        if (!isCurrentBlank) {
          ...
          // Case A: Numerical mark entered or changed
          ...
          // Case B: Previously graded, now cleared to blank -> DELETE
          if (!isInitialBlank) {
            studentsToDelete.push(sId);
          }
          // Case C: Never graded and still blank -> Do nothing
        }
      });

      // If nothing has changed, show success without unnecessary network calls
      if (gradesToUpsert.length === 0 && studentsToDelete.length === 0) {
      ...
      studentsToDelete.forEach(studentId => {
        requests.push(deleteAssessmentGrade(activeAssessment.id, studentId));
      });

      // POST bulk numerical grades
      if (gradesToUpsert.length > 0) {
        requests.push(bulkUpsertAssessmentGrades(activeAssessment.id, { grades: gradesToUpsert }));
      }
          ...
          {assessments.length === 0 ? (
            ...
            <div className="space-y-3">
              {assessments.map(assessment => {
                ...
                let gradedCount = 0;
                if (isActive) {
                  gradedCount = Object.values(grades).filter(v => v !== '' && v !== undefined && v !== null).length;
                } else if (assessment.grades) {
                  gradedCount = Object.keys(assessment.grades).length;
                }
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {students.length === 0 ? (
                        ...
                        students.map((student) => {
                          ...
                          const percentage = (val !== '' && val !== undefined && val !== null) 
