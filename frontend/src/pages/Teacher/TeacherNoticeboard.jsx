...
export default function TeacherNoticeboard() {
  ...
  const fetchNotices = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load notices from server:', err);
      toast.error('Failed to load notices from server.');
    } finally {
      setLoading(false);
    }
  }, [activeTab, classId, markUnreadAsViewed]);

  // Load classes from REST API for ID-to-name lookup
  useEffect(() => {
    let isMounted = true;
  ...
  // Load students for assigned class from REST API (for specific parents target selection)
  useEffect(() => {
    let isMounted = true;
    async function loadStudents() {
      ...
      try {
        const res = await listStudents({ classId: classId, limit: 100 });
        if (isMounted && res.data) {
  ...
  // Refetch notices on tab or class changes
  useEffect(() => {
    fetchNotices();
  }, [fetchNotices]);
  ...
  const handleSaveNotice = async (e) => {
    ...
    try {
      ...
      setNewNotice({ title: '', message: '', audience: 'all', priority: 'normal', targetStudentIds: [] });
      await fetchNotices();
    } catch (error) {
      console.error('Error saving notice:', error);
      toast.error(error.message || `Failed to ${editingNotice ? 'update' : 'create'} notice.`);
    } finally {
      setCreating(false);
    }
  };
        ...
        <div className="space-y-4">
          {displayedNotices.length === 0 ? (
            ...
            displayedNotices.map((notice) => {
              ...
              const isAuthor = notice.authorId === currentUser?.uid || notice.authorId === userProfile?.id;
                ...
                >
                  <div className="flex-1 space-y-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200">
                          ...
                          {activeTab === 'class' && notice.classId ? ` (Class: ${classesMap[notice.classId] || notice.className || notice.classId})` : ''}
                        </span>
                    ...
                    <div className="text-sm font-medium text-slate-400 dark:text-slate-300 pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                      ...
                      Posted by: {notice.authorName || 'Teacher'}
                    </div>
                  </div>
