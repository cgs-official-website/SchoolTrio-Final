...
import { listLessonPlans, createLessonPlan, updateLessonPlan, deleteLessonPlan } from '../../api/lesson-plans';
 ...
 * Avoids UTC timezone conversion drift.
 */
...
/**
 * Formats a date string (YYYY-MM-DD) into UK locale (DD/MM/YYYY) without timezone drift.
 */
...
export default function LessonPlans() {
    ...
    const exportData = plans.map((plan, index) => {
      ...
      availableFieldsList.forEach(field => {
        if (selectedFields[field.key]) {
          if (field.key === 'date') {
            row[field.label] = formatDateDisplay(plan.date);
          } else if (field.key === 'class') {
            row[field.label] = plan.className || plan.class || '';
          } else if (field.key === 'subject') {
            row[field.label] = plan.subjectName || plan.subject || '';
          } else {
  ...
  // Fetch lesson plans via REST API (paginated loop ensures full dataset completeness)
  const fetchLessonPlans = useCallback(async (silent = false) => {
    ...
    try {
      ...
      do {
        ...
        allPlans = allPlans.concat(pageData);
      ...
      console.error('Error fetching lesson plans from REST API:', err);
      toast.error(err.message || 'Failed to load lesson plans');
      setPlans([]);
    } finally {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      if (!schoolId || !currentUser) return;
      if (!isMounted) return;
      await Promise.all([fetchMetadata(), fetchLessonPlans()]);
    };
  ...
  const handleOpenEdit = (plan) => {
    ...
    setFormData({
      ...
      topic: plan.topic || '',
      date: plan.date ? String(plan.date).substring(0, 10) : '',
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    if (!formData.classId || !formData.subjectId || !formData.topic || !formData.date) {
    ...
    try {
      ...
      if (editingPlan) {
        const res = await updateLessonPlan(editingPlan.id, payload);
  ...
  const filteredPlans = plans.filter(p => {
    const matchesSearch = 
      ...
      (p.subjectName || p.subject || '').toLowerCase().includes(searchTerm.toLowerCase());
    
    const planDateStr = p.date ? String(p.date).substring(0, 10) : '';
    const todayStr = getTodayDateString();
    
    const isPast = (planDateStr && planDateStr < todayStr) || p.status === 'completed';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPlans.map(plan => (
                <div key={plan.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-primary-300 dark:hover:border-slate-700 hover:shadow-md transition-all group flex flex-col">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 mb-2 inline-block">
                        {plan.className || plan.class || 'Unknown Class'}
                      </span>
                  ...
                  <p className="text-sm font-semibold text-primary-600 mb-6 flex-1">{plan.subjectName || plan.subject || 'Unknown Subject'}</p>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-4">
              ...
              <div>
                ...
                {subjects.length > 0 ? (
                  ...
                  <input
                    type="text"
                    placeholder="Subject ID (e.g. UUID)"
                    value={formData.subjectId}
                    onChange={(e) => setFormData({ ...formData, subjectId: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary-500 focus:outline-none text-sm font-medium"
                  />
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Objectives / Notes (Optional)</label>
                <textarea
                  placeholder="e.g. Introduce fundamental theorems and practice sample problems"
                  rows={3}
                  value={formData.objectives}
                  onChange={(e) => setFormData({ ...formData, objectives: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary-500 focus:outline-none text-sm font-medium resize-none"
                />
              </div>
            </div>
            {/* Modal Footer */}
              ...
              <div className="flex justify-between items-center w-full">
                {editingPlan && hasDeletePermission ? (
                  <button
                    ...
                    onClick={() => {
                      setShowModal(false);
                      handleDeleteClick(editingPlan.id, editingPlan.topic);
                    }}
