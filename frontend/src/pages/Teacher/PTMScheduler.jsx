...
export default function PTMScheduler() {
  ...
  const fetchMeetings = useCallback(async () => {
    try {
      const res = await ptmApi.getTeacherAppointments(classId ? { classId } : {});
      const data = res?.data || [];
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      const res = await listStudents({ classId, limit: 100, status: 'Active' });
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      setLoading(true);
      await Promise.all([fetchMeetings(), fetchStudents()]);
  ...
  const handleBookMeeting = async (e) => {
    ...
    try {
      ...
      const response = await ptmApi.createAppointment(createPayload);
      const createdAppointment = response?.data || {};
      const ptmId = createdAppointment.id;
      ...
      const ptmPayload = {
        ...
        parentName: student.parentName || student.guardianName || 'Parent',
        date: newMeeting.date,
        time: newMeeting.time,
        type: newMeeting.type,
        status: 'confirmed'
      ...
      const msg = error?.message || 'Failed to book meeting';
      toast.error(msg);
    } finally {
      setCreating(false);
    }
            ...
            {displayedMeetings.map(meeting => (
              <div key={meeting.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-primary-300 dark:hover:border-slate-700 hover:shadow-md transition-all flex flex-col group">
                ...
                <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 mb-4 flex-1">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{new Date(meeting.date).toLocaleDateString('en-GB')} at {formatTime12hr(meeting.time || meeting.timeSlot)}</p>
                    </div>
                    <div>
                ...
                <div className="flex justify-between items-center pt-2">
                  ...
                  ) : meeting.status?.toLowerCase() === 'confirmed' && activeTab === 'upcoming' ? (
