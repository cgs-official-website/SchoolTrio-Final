...
import { LuUsers as Users, LuSearch as Search, LuGraduationCap as GraduationCap, LuCircleCheck as CheckCircle2, LuBus, LuUserCheck, LuUserX, LuUser, LuUserRound, LuFileDown, LuX } from 'react-icons/lu';
  ...
  useEffect(() => {
    ...
    const fetchClassAndStudents = async () => {
      try {
        ...
        const studentsList = Array.isArray(studentsRes?.data) ? studentsRes.data : (Array.isArray(studentsRes) ? studentsRes : []);
        const sorted = [...studentsList].sort((a, b) => (a.firstName || '').localeCompare(b.firstName || ''));
        setStudents(sorted);
      } catch (err) {
        console.error('Error fetching class and students in ClassRoster:', err);
      } finally {
    ...
    // Fetch Transport Routes via REST
    listRoutes().then(res => {
      const routes = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
      setTransportRoutes(routes);
    }).catch(console.error);

    // Fetch Today's Forenoon Attendance via REST API
    ...
    const loadTodayAttendance = async () => {
      try {
        ...
        if (todaySession) {
          let full = todaySession;
          if (!todaySession.records || todaySession.records.length === 0) {
            const detailRes = await getAttendanceSession(todaySession.id);
            if (!isAttendanceMounted) return;
            full = detailRes?.data || detailRes?.session || detailRes || todaySession;
          }
          const recs = {};
          (full.records || []).forEach(r => {
            if (r.studentId) recs[r.studentId] = r.status;
          });
          if (isAttendanceMounted) setAttendanceRecords(recs);
        } else {
          if (isAttendanceMounted) setAttendanceRecords({});
        }
      } catch (err) {
        console.error('Error fetching today attendance in ClassRoster:', err);
        if (isAttendanceMounted) setAttendanceRecords({});
      }
    };
    loadTodayAttendance();
                  ...
                  <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600 dark:text-slate-300">
                      {(() => {
                         ...
                         const vehNo = matchedRoute?.vehicle?.registrationNumber || matchedRoute?.routeNumber || matchedRoute?.vehicleNumber;
                         return vehNo ? (
