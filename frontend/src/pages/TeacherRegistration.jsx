...
import { registerTeacher } from '../api/registration';
  ...
  React.useEffect(() => {
    const fetchSchool = async () => {
      try {
        ...
        if (res?.data) {
          setSchool(res.data);
          if (res.data.staffFormConfig) {
            setFormConfig(res.data.staffFormConfig);
          }
        }
      } catch (err) {
        console.error(err);
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const payload = {
        ...
        phone: customFieldsData.phone ? customFieldsData.phone.trim() : undefined,
