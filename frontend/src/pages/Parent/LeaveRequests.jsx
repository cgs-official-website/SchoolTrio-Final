...
import { LuPlus, LuCalendar, LuClock, LuCircleCheck, LuCircleX, LuFolderDown, LuX, LuDownload } from 'react-icons/lu';
...
export default function LeaveRequests() {
  ...
  const resolvedStudentName = activeChild?.name || 
  ...
  // Fetch student leave applications from REST API
  const fetchLeaves = async (targetStudentId) => {
    if (!targetStudentId) {
      setLeaves([]);
      setLoading(false);
      return;
    ...
    try {
      setLoading(true);
      const res = await getStudentLeaves(targetStudentId, { limit: 100, order: 'desc' });
      if (!mountedRef.current || currentStudentRef.current !== targetStudentId) return;

      const rawLeaves = Array.isArray(res?.data) ? res.data : [];
      setLeaves(rawLeaves);
    } catch (error) {
      if (mountedRef.current && currentStudentRef.current === targetStudentId) {
        console.error('[LeaveRequests] Error loading student leaves:', error);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!studentId) {
      toast.error('No active student selected.');
      return;
    }
    ...
    if (formData.leaveType === 'others' && !formData.customType.trim()) {
    ...
    try {
      ...
      if (formData.leaveType === 'others') {
        finalLeaveType = formData.customType.trim();
      } else {
      ...
      let docInfo = null;
      if (selectedFile) {
        const storagePath = `schools/${schoolId || 'leaves'}/leaves/${studentId}_${Date.now()}_${selectedFile.name}`;
      ...
      console.error('[LeaveRequests] Error submitting leave:', error);
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto h-full flex flex-col animate-fade-in-up min-w-0 w-full">
      ...
      <div className="mb-8 shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 min-w-0 w-full">
        <div className="min-w-0 w-full">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Submit leaves on behalf of {resolvedStudentName} and track approval status.</p>
        </div>
        <button 
