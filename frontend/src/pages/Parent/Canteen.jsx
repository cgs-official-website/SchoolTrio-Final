...
export default function Canteen() {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      setLoading(true);
      const res = await listCanteenRequests(studentId ? { studentId } : {});
      const reqList = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
      setRequests(studentId ? reqList.filter(r => r.studentId === studentId) : reqList);
    } catch (error) {
  ...
  const handleRequestMeal = async (mealType) => {
    ...
    try {
      ...
      const errorMessage = error?.message || "Failed to request meal.";
          ...
          <div className="w-full min-w-0 overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-max">
              ...
              <tbody className="divide-y divide-slate-100">
                {requests.slice().sort((a,b) => new Date(b.createdAt || b.timestamp || 0) - new Date(a.createdAt || a.timestamp || 0)).map((req) => (
