...
import { 
  Plus, X, Search, Edit, Trash2, Download, Upload, Loader2
} from 'lucide-react';
...
export default function InventoryManagement() {
  ...
  // Selected items for export / bulk delete
  ...
  // Form states
  const [itemFormData, setItemFormData] = useState({ id: '', productId: '', name: '', category: '', categoryId: '', quantity: 0, unit: 'pcs', status: 'In Stock', customData: {} });
  ...
  // Load inventory items and categories via REST
  const loadInventoryData = useCallback(async () => {
    if (!schoolId) return;
    setLoading(true);
    try {
      const [itemsData, categoriesRes] = await Promise.all([
        fetchAllPages(listItems, {}, 100),
        listCategories()
      ]);
      startTransition(() => {
  ...
  const handleSaveItem = async (e) => {
    ...
    const prodIdTrimmed = itemFormData.productId ? itemFormData.productId.trim() : '';
    if (prodIdTrimmed) {
      const isDuplicate = items.some(i => i.id !== itemFormData.id && i.productId && i.productId.toLowerCase() === prodIdTrimmed.toLowerCase());
      if (isDuplicate) {
    ...
    const qty = parseInt(itemFormData.quantity, 10) || 0;
    ...
    try {
      ...
      if (itemFormData.id) {
        // Edit Item (Metadata ONLY - quantity must NOT be sent)
        const updatePayload = {
          name: nameTrimmed,
          productId: prodIdTrimmed || null,
  ...
  const handleSaveCategory = async (e) => {
    ...
    const finalData = {
      name: catName,
      description: categoryFormData.description ? categoryFormData.description.trim() : null
    };
  ...
  // Stock Adjustment (Inbound / Outbound)
  const handleStockAdjustment = async (e) => {
    e.preventDefault();
    if (!schoolId) return;

    const qtyVal = parseInt(stockAdjustment.quantity, 10) || 0;
    if (qtyVal <= 0) {
      toast.error('Quantity must be greater than 0');
      return;
    }

    if (stockAdjustment.type === 'outbound' && stockAdjustment.prevStock < qtyVal) {
    ...
    setActionLoading(true);
    try {
      await adjustStock(stockAdjustment.itemId, {
        type: stockAdjustment.type,
        quantity: qtyVal,
        remarks: stockAdjustment.remarks ? stockAdjustment.remarks.trim() : null
      });
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedItemIds.length} selected items? Items with transaction history cannot be deleted.`)) {
      return;
    }
    ...
    try {
      ...
      if (data.blockedCount > 0) {
        toast.success(`Deleted ${data.deletedCount} items. ${data.blockedCount} items blocked due to existing audit logs.`);
      } else {
  ...
  const filteredItems = items.filter(i => {
    ...
    const matchesSearch = !searchTerm || 
                          ...
                          (i.productId && i.productId.toLowerCase().includes(sTerm)) ||
                          (i.category && i.category.toLowerCase().includes(sTerm));
  ...
  const handleBulkExport = async (format) => {
    ...
    if (format === 'csv') {
      ...
      link.click();
      toast.success('Inventory products exported to CSV');
    } else {
      ...
      toast.success('Inventory products exported to Excel');
    }
  ...
  const handleProcessImport = () => {
    ...
    reader.onload = async (evt) => {
      ...
      for (let rIdx = 1; rIdx < data.length; rIdx++) {
        ...
        const rawStock = idxStock !== -1 ? parseInt(row[idxStock], 10) : 0;

        rowsToImport.push({
          productId: rawId || null,
          name: rawName,
          category: rawCat || 'Uncategorized',
          quantity: isNaN(rawStock) ? 0 : Math.max(0, rawStock)
        });
      }

      if (rowsToImport.length === 0) {
        toast.error('Uploaded file has no valid data rows');
        return;
      }
      ...
      try {
        const res = await bulkImportItems({
          items: rowsToImport,
          autoCreateCategories: importConfig.autoCreateCategories,
          duplicateAction: importConfig.duplicateAction
        });

        const summary = res?.data || {};
        setImportSummary({
          totalRows: summary.totalRows || rowsToImport.length,
  ...
  const downloadErrorReport = () => {
    ...
    const csvContent = 'Row Number,Validation Error Message\n' + 
      importErrors.map(e => `${e.row || e.index || '—'},"${(e.message || String(e)).replace(/"/g, '""')}"`).join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.setAttribute('href', url);
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto flex flex-col min-w-0 min-h-screen pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 shrink-0 w-full">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          {selectedItemIds.length > 0 && canDelete('inventory') && (
            <button 
              onClick={handleBulkDelete}
              disabled={actionLoading}
              className="w-full sm:w-auto justify-center flex items-center gap-2 bg-red-600 text-white px-4 py-2 rounded-xl hover:bg-red-700 transition-all font-semibold shadow-sm disabled:opacity-50"
            >
              <Trash2 size={18} /> Delete Selected ({selectedItemIds.length})
            </button>
          )}
          ...
          {activeTab === 'items' ? (
            canCreate('inventory') ? (
              <button 
                onClick={() => {
                  setItemFormData({ id: '', productId: '', name: '', category: '', categoryId: '', quantity: 0, unit: 'pcs', status: 'In Stock', customData: {} });
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {paginatedItems.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 font-bold text-slate-900 dark:text-white">{item.quantity} {item.unit || 'pcs'}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {canDelete('inventory') && (
                            <button 
                              onClick={() => setConfirmDeleteState({ isOpen: true, type: 'item', id: item.id, name: item.name })}
                  ...
                  <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {canDelete('inventory') && (
                          <button 
                            onClick={() => setConfirmDeleteState({ isOpen: true, type: 'category', id: cat.id, name: cat.name })}
                            className="p-2 text-slate-400 dark:text-slate-300 hover:text-red-600 hover:bg-red-50 dark:hover:bg-slate-800 rounded-lg transition-colors"
                          >
                            <Trash2 size={16} />
                          </button>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">
                    Initial Stock Quantity {itemFormData.id ? '(Adjust stock using "Adjust Stock" button)' : ''}
                  </label>
                  <input 
                    type="number" required min="0"
                    disabled={!!itemFormData.id}
                    value={itemFormData.quantity} 
                    onChange={e => setItemFormData({...itemFormData, quantity: parseInt(e.target.value, 10) || 0})}
                  ...
                  <div className="flex-1 text-center">
                    ...
                    <span className="text-xl font-bold text-primary-600">
                      {stockAdjustment.type === 'inbound' 
                        ? stockAdjustment.prevStock + (parseInt(stockAdjustment.quantity, 10) || 0)
                        : Math.max(0, stockAdjustment.prevStock - (parseInt(stockAdjustment.quantity, 10) || 0))
                      }
                    </span>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fade-in-up">
                ...
                <div className="border border-red-200 bg-red-50/50 p-4 rounded-2xl space-y-2">
                  ...
                  <div className="max-h-32 overflow-y-auto text-xs text-red-600 space-y-1 custom-scrollbar">
                    {importErrors.map((e, idx) => (
                      <p key={idx}>• Row {e.row || e.index || idx + 1}: {e.message || String(e)}</p>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3 shrink-0">
              ...
              <button 
                ...
                className="w-full sm:w-auto px-4 py-2 bg-primary-600 text-white text-sm font-semibold hover:bg-primary-700 rounded-xl shadow-sm transition-colors disabled:opacity-50"
              >
                {actionLoading ? 'Importing...' : 'Import Data'}
              </button>
            </div>
          </div>
        </div>
