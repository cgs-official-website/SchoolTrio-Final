...
import { listCollectionPeriods, createCollectionPeriod, updateCollectionPeriod, deleteCollectionPeriod } from '../../api/fees';
import { listLeaveApprovalRules, createLeaveApprovalRule, updateLeaveApprovalRule, deleteLeaveApprovalRule } from '../../api/leaves';
...
export default function EnvironmentSetup() {
  ...
  const [periodForm, setPeriodForm] = useState({ name: '', dueDate: new Date().toISOString().split('T')[0], displayOrder: '' });
  ...
  // Load fee collection periods via REST
  const fetchFeePeriods = async () => {
    try {
      const res = await listCollectionPeriods({ limit: 100 });
      setPeriods(Array.isArray(res?.data) ? res.data : []);
    } catch (err) {
      console.error('[EnvironmentSetup] Error fetching fee collection periods:', err);
    }
  };

  useEffect(() => {
    if (schoolId) fetchFeePeriods();
  }, [schoolId]);

  // Load leave approval rules via REST
  const fetchLeaveRules = async () => {
    try {
      const res = await listLeaveApprovalRules();
      setLeaveRules(Array.isArray(res?.data) ? res.data : []);
    } catch (err) {
      console.error('[EnvironmentSetup] Error fetching leave approval rules:', err);
    }
  };

  useEffect(() => {
    if (schoolId) fetchLeaveRules();
  }, [schoolId]);

  // Fetch roles list for dropdown via REST
  ...
  const openAddPeriod = () => {
    ...
    setPeriodForm({ name: '', dueDate: new Date().toISOString().split('T')[0], displayOrder: periods.length + 1 });
    setShowPeriodModal(true);
  };

  const openEditPeriod = (p) => {
    setEditingPeriod(p);
    setPeriodForm({ name: p.name, dueDate: p.dueDate || new Date().toISOString().split('T')[0], displayOrder: p.displayOrder || '' });
    setShowPeriodModal(true);
  };
  ...
  const handleSavePeriod = async (e) => {
    ...
    if (!periodForm.dueDate) return toast.error('Due date is required.');
    ...
    try {
      ...
      if (editingPeriod) {
        await updateCollectionPeriod(editingPeriod.id, payload);
        toast.success('Period updated successfully.');
      } else {
        await createCollectionPeriod(payload);
        toast.success('Fee collection period added.');
      }
      setShowPeriodModal(false);
      await fetchFeePeriods();
    } catch (err) {
      console.error(err);
  ...
  const handleDeletePeriod = (p) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          await deleteCollectionPeriod(p.id);
  ...
  const handleSaveAttendanceConfig = async (e) => {
    ...
    try {
      await updateAttendanceSettings(attendanceConfig);
  ...
  const handleSaveLeaveRule = async (e) => {
    ...
    try {
      ...
      if (editingLeaveRule) {
        await updateLeaveApprovalRule(editingLeaveRule.id, payload);
  ...
  const handleDeleteLeaveRule = (rule) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          await deleteLeaveApprovalRule(rule.id);
  ...
  const fetchSchoolData = async () => {
    try {
      ...
      // Fetch Attendance Settings (REST)
      try {
        const attRes = await getAttendanceSettings();
        const attSettings = attRes?.data || attRes;
        if (attSettings) {
          setAttendanceConfig({
        ...
        console.error('Error fetching attendance settings in EnvironmentSetup:', err);
      }

      // Fetch Schema (REST)
      try {
        const schemaRes = await getFormSchema('environment_setup');
    ...
    <div className="p-8 max-w-5xl mx-auto pb-24">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          {periods.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {periods.map((p) => (
                <div key={p.id} className="flex items-center justify-between px-6 py-4 hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="w-9 h-9 rounded-xl bg-primary-50 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 flex items-center justify-center font-black text-sm shrink-0">
                      {p.displayOrder || '—'}
                    </div>
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">{p.name}</p>
                      {p.dueDate && <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Due Date: <span className="font-medium text-slate-700 dark:text-slate-300">{p.dueDate}</span></p>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => openEditPeriod(p)} className="p-2 text-slate-400 dark:text-slate-300 hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Edit Period">
                      <Pencil size={15} />
                    </button>
                    <button type="button" onClick={() => handleDeletePeriod(p)} className="p-2 text-slate-400 dark:text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Delete Period">
                      <Trash size={15} />
                    </button>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                        Route to Role: <span className="font-semibold text-indigo-600">{rule.role?.name || roles.find(r => r.id === rule.roleId)?.name || rule.roleId}</span>
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in-up">
            ...
            <form onSubmit={handleSavePeriod} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">Due Date <span className="text-red-500">*</span></label>
                  <input
                    type="date" required
                    value={periodForm.dueDate}
                    onChange={(e) => setPeriodForm(p => ({ ...p, dueDate: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary-500"
                  />
                </div>
                <div>
      ...
      <ConfirmModal
        ...
        onConfirm={async () => {
          ...
          setConfirmModal({ isOpen: false, onConfirm: null, message: '', title: '' });
        }}
        onClose={() => setConfirmModal({ isOpen: false, onConfirm: null, message: '', title: '' })}
      />
    </div>
  );
}
