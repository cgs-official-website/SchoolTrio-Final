...
export default function CustomModuleView() {
  ...
  const loadModuleData = async () => {
    ...
    try {
      ...
      try {
        const metaRes = await getCustomModule(moduleId);
        setModuleMetadata(metaRes?.data || { name: 'Custom Module' });
      } catch {
        setModuleMetadata({ name: 'Custom Module' });
      }

      // 2. Fetch Form Schema via REST
      try {
        const schemaRes = await getFormSchema(moduleId);
        const schemaData = schemaRes?.data || schemaRes;
        setSchema(schemaData?.sections || []);
      } catch {
        setSchema([]);
      }

      // 3. Fetch Data Collection via REST
      try {
        const recordsRes = await listModuleRecords(moduleId, { limit: 100 });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Clean payload for record data
      const cleanData = { ...finalFormData };
      delete cleanData.id;
      delete cleanData.createdAt;
      delete cleanData.updatedAt;
      delete cleanData.schoolId;
      delete cleanData.customModuleId;

      // 2. Save data via REST
      if (editingId) {
        await updateModuleRecord(moduleId, editingId, cleanData);
        setData(data.map(d => d.id === editingId ? { id: editingId, ...cleanData } : d));
        toast.success("Record updated");
      } else {
        const createRes = await createModuleRecord(moduleId, cleanData);
        const newRecordId = createRes?.data?.id || `rec_${Date.now()}`;
