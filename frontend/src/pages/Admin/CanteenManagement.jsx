...
import { listCanteenRequests, updateCanteenRequestStatus } from '../../api/canteen';
...
export default function CanteenManagement() {
  ...
  // Fetch Canteen Requests via REST API
  const fetchRequests = useCallback(async () => {
    try {
      setLoading(true);
      const res = await listCanteenRequests();
      const reqList = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
      setRequests(reqList);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (studentsRes.status === 'fulfilled') {
        const list = Array.isArray(studentsRes.value?.data) ? studentsRes.value.data : [];
        setStudents(list);
      }
      if (classesRes.status === 'fulfilled') {
        const list = Array.isArray(classesRes.value?.data) ? classesRes.value.data : (Array.isArray(classesRes.value) ? classesRes.value : []);
        setClasses(sortClassesAscending(list));
      }
    }).catch(err => {
      console.error('[CanteenManagement] Error fetching auxiliary data:', err);
    });
  ...
  // Handle Status Update via REST API
