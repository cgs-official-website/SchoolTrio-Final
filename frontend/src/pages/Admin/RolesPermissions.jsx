...
const ADMIN_PANEL_MODULES = [
  ...
  'health', 'complaints', 'alumni', 'documents', 'branches', 'reports', 'form-builder', 'api', 'links', 'leads', 'billing', 'chats', 'homework', 'leaves', 'lesson_plans', 'resources', 'ptm', 'performance', 'media'
];
...
// Build unified canonical module display catalog (deduplicated by id)
const ALL_MODULES_MAP = new Map();
[...CORE_MODULES, ...ALL_AVAILABLE_MODULES].forEach(m => {
  if (!ALL_MODULES_MAP.has(m.id)) {
    ALL_MODULES_MAP.set(m.id, m);
  }
});
const UNIFIED_MODULES_LIST = Array.from(ALL_MODULES_MAP.values());
...
export default function RolesPermissions() {
  ...
  const [permissions, setPermissions] = useState({}); // { [roleName]: { [moduleKey]: { read, create, edit, delete } } }
  const [loginPanels, setLoginPanels] = useState({}); // { [roleName]: 'admin' | 'teacher' }
  const [rolesMap, setRolesMap] = useState({}); // { [roleName]: roleObjectWithId }
  const [displayModules] = useState(UNIFIED_MODULES_LIST);
    ...
    try {
      ...
      if (res && res.success && Array.isArray(res.data)) {
        ...
        res.data.forEach(role => {
          ...
          fetchedMap[roleName] = role;
          fetchedPanels[roleName] = role.loginPanel || 'admin';

          const rolePerms = {};
          if (Array.isArray(role.permissions)) {
            role.permissions.forEach(p => {
              rolePerms[p.moduleKey] = {
                read: Boolean(p.canRead),
                create: Boolean(p.canCreate),
                edit: Boolean(p.canEdit),
                delete: Boolean(p.canDelete)
              };
            });
          } else if (role.permissions && typeof role.permissions === 'object') {
            Object.entries(role.permissions).forEach(([key, p]) => {
              rolePerms[key] = {
                read: Boolean(p.canRead ?? p.read),
                create: Boolean(p.canCreate ?? p.create),
                edit: Boolean(p.canEdit ?? p.edit),
                delete: Boolean(p.canDelete ?? p.delete)
              };
            });
          }
          fetchedPerms[roleName] = rolePerms;
        });

        // Ensure default roles are always present in the list
        const uniqueRoles = Array.from(new Set([...DEFAULT_ROLES, ...allRoles]));
        setRolesList(uniqueRoles);
        setPermissions(fetchedPerms);
        setLoginPanels(fetchedPanels);
        setRolesMap(fetchedMap);

        if (!uniqueRoles.includes(activeRole)) {
          setActiveRole(uniqueRoles[0] || DEFAULT_ROLES[0]);
        }
      }
    } catch (error) {
      console.error('Error fetching roles from REST API:', error);
  ...
  const handleAddRole = async (e) => {
    ...
    if (!role) return;
    if (rolesList.some(r => r.toLowerCase() === role.toLowerCase())) {
    ...
    try {
      ...
      if (res && res.success && res.data) {
        ...
        setRolesMap(prev => ({ ...prev, [role]: createdRole }));
        setRolesList(prev => [...prev, role]);
        setLoginPanels(prev => ({ ...prev, [role]: createdRole.loginPanel || 'admin' }));
  ...
  const handleToggle = (moduleKey, action) => {
    setPermissions(prev => {
      ...
      // Invariant 1: If turning on create/edit/delete, automatically turn on read
      if ((action === 'create' || action === 'edit' || action === 'delete') && newModulePerms[action]) {
        newModulePerms.read = true;
      }
      // Invariant 2: If turning off read, automatically turn off create/edit/delete
  ...
  const handleDeleteRole = (e, roleToDelete) => {
    e.stopPropagation();
    const roleObj = rolesMap[roleToDelete];
    if (DEFAULT_ROLES.includes(roleToDelete) || roleObj?.isSystemDefault) {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setRolesMap(prev => {
            const newMap = { ...prev };
            delete newMap[roleToDelete];
            return newMap;
          });
          setLoginPanels(prev => {
            const newPanels = { ...prev };
            delete newPanels[roleToDelete];
            return newPanels;
          });

          if (activeRole === roleToDelete) {
            const remaining = rolesList.filter(r => r !== roleToDelete);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const roleObj = rolesMap[activeRole];

      if (roleObj?.id) {
        // 1. Update permissions
        await updateRolePermissions(roleObj.id, { permissions: rolePerms });

        // 2. Update metadata if loginPanel changed
        if (roleObj.loginPanel !== activePanel) {
          await updateRole(roleObj.id, { loginPanel: activePanel });
        }
      } else {
        // If role doesn't exist yet on backend, create it
        const res = await createRole({
          name: activeRole,
        ...
        if (res && res.data) {
          setRolesMap(prev => ({ ...prev, [activeRole]: res.data }));
        }
      }
