...
import { getPlans, getCurrentBilling, upgradePlan } from '../../api/billing';
import {
  ...
  const fetchData = async () => {
    ...
    try {
      const [plansRes, currentRes] = await Promise.all([
        getPlans(),
        getCurrentBilling().catch(() => ({ data: null }))
      ]);

      const allPlans = plansRes?.data || [];
      const activePlans = allPlans.filter(plan => plan.isActive !== false && plan.active !== false);
      setPlans(activePlans.sort((a, b) => (a.pricePerUserPerYear || 0) - (b.pricePerUserPerYear || 0)));

      const currentPlan = currentRes?.data?.plan;
      setCurrentPlanId(currentPlan?.id || currentRes?.data?.planId || null);
      if (currentRes?.data?.billingCycle) {
        setBillingCycle(currentRes.data.billingCycle);
      }
    } catch (err) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto pb-24">
      <div className="text-center max-w-2xl mx-auto mb-16 mt-8">
        ...
        {error && (
          <div className="mt-6 p-4 rounded-xl bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm">
            {error}
          </div>
        )}
      ...
      {upgrading ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl p-16 text-center max-w-md mx-auto">
          ...
          <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Processing Subscription...</h3>
