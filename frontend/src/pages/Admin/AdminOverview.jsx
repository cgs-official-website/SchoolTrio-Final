...
import { listPayroll } from '../../api/hr-payroll';
...
export default function AdminOverview() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      if (userProfile?.schoolId || userProfile?.uid) {
        try {
          const res = await getSchoolSettings();
          const savedConfig = res?.data?.customData?.dashboardConfig || res?.customData?.dashboardConfig;
          if (savedConfig) {
  ...
  const handleDismissAlert = async (alertId) => {
    try {
      await notificationsApi.markNotificationRead(alertId);
      setPendingAttendanceAlerts(prev => prev.filter(a => a.id !== alertId));
