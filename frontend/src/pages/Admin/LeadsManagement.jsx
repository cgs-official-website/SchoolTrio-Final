...
export default function LeadsManagement() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [leadsRes, formsRes] = await Promise.all([
        admissionsApi.getLeads({ limit: 500 }),
        admissionsApi.getLeadForms({ limit: 100 })
      ]);

      const rawLeads = Array.isArray(leadsRes?.data) ? leadsRes.data : [];
      const normalizedLeads = rawLeads.map(lead => ({
        ...lead,
        data: lead.customData || lead.data || {},
        formTitle: lead.form?.title || lead.formTitle || 'Untitled Form'
      }));
      normalizedLeads.sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt));
      setLeads(normalizedLeads);

      const rawForms = Array.isArray(formsRes?.data) ? formsRes.data : [];
      const normalizedForms = rawForms.map(form => {
        const fieldList = Array.isArray(form.fields?.fieldList)
          ? form.fields.fieldList
          : Array.isArray(form.fields)
            ? form.fields
            : [];
        const successMessage = form.fields?.successMessage || form.successMessage || '';
        return {
          ...form,
          fields: fieldList,
          successMessage
      ...
      console.error('Failed to load leads or forms:', err);
  ...
  // Form builder: save to PostgreSQL via REST API
  const handleSaveForm = async () => {
    ...
    try {
      if (editingForm.id === 'new') {
        await admissionsApi.createLeadForm(payload);
