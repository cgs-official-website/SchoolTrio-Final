...
import { LuSearch as Search, LuShieldCheck as ShieldCheck, LuMail as Mail, LuUsers as Users, LuCircleCheck as CheckCircle2, LuX as X, LuCloudUpload as UploadCloud, LuFileText as FileText, LuLink as LinkIcon, LuDownload as Download, LuFileSpreadsheet as FileSpreadsheet, LuUserPlus as UserPlus, LuEye as Eye, LuFilter as Filter, LuTrash2 as Trash, LuFileDown as FileDown, LuUser as User, LuGraduationCap as GraduationCap, LuBriefcase as Briefcase, LuBuilding2 as Building2, LuCreditCard as CreditCard, LuChevronRight as ChevronRight, LuChevronLeft as ChevronLeft } from 'react-icons/lu';
...
function normalizeStaffMember(s) {
  ...
  return {
    ...
    mobileNumber: s.phone || custom.mobileNumber || '',
    phone: s.phone || custom.mobileNumber || '',
    staffId: s.employeeId || custom.staffId || '',
    employeeId: s.employeeId || custom.staffId || '',
    ...
    roleId,
    designation: s.designation || roleName,
    dob: s.dob || custom.dob || '',
    gender: s.gender || custom.gender || 'Male',
    ...
    esicNumber: fin.esicNumber || custom.esicNumber || '',
    uanNumber: fin.uanNumber || custom.uanNumber || '',
    taxIdDetails: fin.taxIdDetails || custom.taxIdDetails || '',
    bankName: fin.bankName || custom.bankName || '',
    bankAccountNumber: fin.bankAccountNumber || custom.bankAccountNumber || '',
    ...
    markSheets: docs.markSheets || custom.markSheets || [],
    experienceCertificates: docs.experienceCertificates || custom.experienceCertificates || [],
    relievingLetter: docs.relievingLetter || custom.relievingLetter || [],
    resume: docs.resume || custom.resume || [],
...
export default function StaffAssignment() {
  ...
  const [schoolName, setSchoolName] = useState(userProfile?.schoolName || 'School');
  ...
  const handleDeleteStaff = async (staffId) => {
    try {
      ...
      if (error.status === 409 || error.message?.includes('409') || error.message?.toLowerCase().includes('cannot delete')) {
        toast.error(error.message || "Cannot delete staff member with active class assignments or historical records. Please deactivate the account instead.", { duration: 6000 });
      } else {
  ...
  const fetchStaffData = useCallback(async () => {
    ...
    try {
      ...
      const normalizedStaff = rawStaff.map(normalizeStaffMember);
      setStaff(normalizedStaff);

      const rawClasses = Array.isArray(classesRes?.data) ? classesRes.data : (classesRes?.data?.classes || classesRes?.data?.data || []);
      setClasses(sortClassesAscending(rawClasses));

      const rawSubjects = Array.isArray(subjectsRes?.data) ? subjectsRes.data : (subjectsRes?.data?.subjects || subjectsRes?.data?.data || []);
      setSubjects(rawSubjects);

      const rawRoles = Array.isArray(rolesRes?.data) ? rolesRes.data : [];
      setRolesList(rawRoles);
      if (rawRoles.length > 0) {
        const roleNames = rawRoles.map(r => r.name).filter(Boolean);
        setAllRoles(prev => {
          const combined = new Set([...prev, ...roleNames]);
          return Array.from(combined);
        });
      }
    } catch (err) {
      console.error("Error loading staff data:", err);
      toast.error(err.message || "Failed to load staff management data.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleAssign = async () => {
    setSaving(true);
    try {
      await assignStaff(selectedStaff.id, {
        ...
        try {
          ...
          for (let i = 0; i < rawData.length; i++) {
            ...
            const staffPayload = {
              ...
              phone: getField(row, 'mobile number', 'mobile', 'phone', 'contact', 'phone number') || null,
              employeeId: getField(row, 'staff id', 'staffid', 'staff_id', 'id') || null,
  ...
  const handleSaveStaffEdit = async () => {
    ...
    try {
      ...
      const selectedRoleName = (editStaffData.roles && editStaffData.roles.length > 0) ? editStaffData.roles[0] : (editStaffData.role || 'Staffs');
      ...
      const updatePayload = {
        ...
        qualifications: {
          ...
          yearOfPassing: (editStaffData.yearOfPassing || '').trim() || null,
        ...
        financial: {
          ...
          uanNumber: (editStaffData.uanNumber || '').trim() || null,
          taxIdDetails: (editStaffData.taxIdDetails || '').trim() || null,
      ...
      const updated = res.data ? normalizeStaffMember(res.data) : { ...selectedStaffToView, ...updatePayload };
  ...
  const exportToExcel = () => {
    ...
    const exportData = staff.map((member, index) => {
      ...
      if (selectedFields.assignedClass) row["Assigned Class"] = getClassName(member.assignedClassId, member);
      if (selectedFields.subjectClasses) row["Subject Classes"] = (member.subjectClassIds || []).map(id => getClassName(id, member)).join(', ');
                        ...
                        <div className="flex flex-wrap gap-1.5 max-w-[280px]">
                          {isAssigned && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded border border-primary-200/60 bg-primary-50/50 text-primary-700 text-[11px] font-semibold uppercase tracking-wide">
                              {getClassName(member.assignedClassId, member)} (Class)
                            </span>
                          )}
      ...
      {addStaffModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh]">
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-700 overflow-x-auto bg-white dark:bg-slate-900 shrink-0 hide-scrollbar scroll-smooth">
              <div className="flex w-full justify-start items-center">
                {[
                  ...
                  { id: 'Government & Identity', label: 'Government & Identity', icon: FileText },
                  { id: 'Employment', label: 'Employment', icon: Building2 },
                  { id: 'Banking', label: 'Banking', icon: CreditCard },
                  { id: 'Uploads', label: 'Uploads', icon: UploadCloud }
                ].map(({ id, label, icon: TabIcon }) => {
                  const isActive = addStaffActiveTab === id;
                  return (
                    <button
                      key={id}
                      type="button"
                ...
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="border border-slate-200 dark:border-slate-700 rounded-2xl p-4 bg-slate-50/50 dark:bg-slate-800/50">
                    <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                      <label className="text-xs font-extrabold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Photograph (JPG/PNG)</label>
                      <label className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-slate-800 border border-indigo-200 dark:border-slate-700 text-indigo-700 dark:text-indigo-300 rounded-xl text-xs font-bold hover:bg-indigo-100 dark:hover:bg-slate-700 cursor-pointer transition-colors shadow-sm">
                        <UploadCloud size={14} />
                        <span>{addStaffFiles.photo ? 'Change Photo' : 'Select Photo'}</span>
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => {
                            if (e.target.files && e.target.files[0]) {
                              setAddStaffFiles(prev => ({ ...prev, photo: e.target.files[0] }));
                            }
                          }}
                          className="hidden"
                        />
                    ...
                    {addStaffFiles.photo ? (
                      <div className="mt-2 flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-xs shadow-sm">
                        <span className="truncate text-slate-700 dark:text-slate-200 font-semibold">{addStaffFiles.photo.name}</span>
                        <button
                          type="button"
                          onClick={() => setAddStaffFiles(prev => ({ ...prev, photo: null }))}
                          className="text-red-500 hover:text-red-700 font-bold px-2 py-1 flex items-center justify-center hover:bg-red-50 dark:hover:bg-slate-800 rounded-lg transition-colors"
                      ...
                      <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium">No photo selected</p>
                    )}
                  </div>
                  ...
                  {[
                    ...
                    <div key={key} className="border border-slate-200 dark:border-slate-700 rounded-2xl p-4 bg-slate-50/50 dark:bg-slate-800/50">
                      <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                        <label className="text-xs font-extrabold text-slate-600 dark:text-slate-300 uppercase tracking-wider">{label}</label>
                        <label className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-slate-800 border border-indigo-200 dark:border-slate-700 text-indigo-700 dark:text-indigo-300 rounded-xl text-xs font-bold hover:bg-indigo-100 dark:hover:bg-slate-700 cursor-pointer transition-colors shadow-sm">
                          <UploadCloud size={14} />
                      ...
                      {(addStaffFiles[key] || []).length > 0 ? (
                        <div className="mt-2 space-y-1.5">
                          {(addStaffFiles[key] || []).map((file, idx) => (
                            <div key={`${key}_${idx}`} className="flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-xs shadow-sm">
                        ...
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium">No files selected</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="p-4 sm:p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-3 shrink-0">
              <div className="flex items-center gap-2">
                {addStaffActiveTab !== 'Personal' && (
                  <button
                    type="button"
                    onClick={() => {
                      const tabs = ['Personal', 'Educational', 'Professional', 'Government & Identity', 'Employment', 'Banking', 'Uploads'];
                      const idx = tabs.indexOf(addStaffActiveTab);
                      if (idx > 0) setAddStaffActiveTab(tabs[idx - 1]);
                    }}
                    className="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-semibold text-xs rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 flex items-center gap-1.5 transition-colors shadow-sm"
                  >
                ...
                {addStaffActiveTab !== 'Uploads' && (
                  <button
                    ...
                    onClick={() => {
                      const tabs = ['Personal', 'Educational', 'Professional', 'Government & Identity', 'Employment', 'Banking', 'Uploads'];
                      const idx = tabs.indexOf(addStaffActiveTab);
                      if (idx < tabs.length - 1) setAddStaffActiveTab(tabs[idx + 1]);
                    }}
                    className="px-4 py-2 bg-indigo-50 dark:bg-slate-700 border border-indigo-200 dark:border-slate-600 text-indigo-700 dark:text-indigo-300 font-semibold text-xs rounded-xl hover:bg-indigo-100 dark:hover:bg-slate-600 flex items-center gap-1.5 transition-colors shadow-sm"
                  >
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-5 py-2.5 text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-sm transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-2.5 bg-primary-600 text-white font-bold hover:bg-primary-700 rounded-xl text-sm shadow-sm disabled:opacity-50 transition-colors flex items-center gap-2"
                >
                  {addingStaff ? (
                    <>
