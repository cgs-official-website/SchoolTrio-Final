...
export default function SchoolRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Deterministic uppercase school code derived from school name
      const baseCode = (formData.schoolName || 'SCH')
        .replace(/[^A-Za-z0-9]/g, '')
        .toUpperCase()
        .slice(0, 4) || 'SCH';
      ...
      const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(selectedPlanDetails?.id);

      // Parse student count
      const seatLimit = parseInt(String(formData.studentCount).replace(/[^0-9]/g, ''), 10) || numUsers || 500;

      const payload = {
        name: formData.schoolName.trim(),
        code: schoolCode,
        type: formData.schoolType || 'School',
        email: formData.email.trim().toLowerCase(),
        phone: formData.phone ? formData.phone.trim() : undefined,
        address: formData.location ? formData.location.trim() : undefined,
        ...(isUuid && { planId: selectedPlanDetails.id }),
        seatLimit: seatLimit > 0 ? seatLimit : 500,
        admin: {
