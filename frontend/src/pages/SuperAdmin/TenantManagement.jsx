...
export default function TenantManagement() {
  ...
  const fetchTenants = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load tenants via REST:', err);
      setSchools([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
  ...
  const openApprovalModal = (school) => {
    ...
    setSelectedModules(school.features || school.permittedModules || []);
    const defaultSeats = school.seatLimit !== undefined ? school.seatLimit : (school.plan?.name?.toLowerCase() === 'enterprise' || school.plan?.toLowerCase() === 'enterprise' ? 2000 : 500);
    const defaultTeachers = school.teacherLimit !== undefined ? school.teacherLimit : (school.plan?.name?.toLowerCase() === 'enterprise' || school.plan?.toLowerCase() === 'enterprise' ? 150 : 50);
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      if (modalAction === 'approve') {
        await updateTenantStatus(selectedSchool.id, { status: 'approved' });
      }

      await updateTenantConfig(selectedSchool.id, {
        maxStudents: parsedSeatLimit,
        maxStaff: parsedTeacherLimit,
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row gap-4 mb-6 shrink-0">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 transition-shadow text-slate-900 dark:text-white"
          />
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {schools.length === 0 ? (
                ...
                schools.map((school) => {
                  ...
                  const email = school.email || school.adminEmail || 'N/A';
                  const planName = school.plan?.name || school.plan || 'Standard';
                  const studentSeats = school.seatLimit || (planName.toLowerCase() === 'enterprise' ? 2000 : 500);
                    ...
                    <tr key={school.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="p-4 text-center">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider
                          ${statusNormalized === 'approved' ? 'bg-green-100 text-green-700 border border-green-200' : 
                            statusNormalized === 'pending' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 
                            'bg-red-100 text-red-700 border border-red-200'}`}
                        >
                          {statusNormalized === 'approved' && <CheckCircle2 size={12} />}
                          {statusNormalized === 'pending' && <AlertCircle size={12} />}
                          {statusNormalized === 'suspended' && <Ban size={12} />}
                          {statusNormalized}
                        </span>
                      </td>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              <div className="mb-6 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white">{selectedSchool.name || selectedSchool.schoolName}</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Purchased Plan: <span className="font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">{selectedSchool.plan?.name || selectedSchool.plan || 'Standard'}</span></p>
                </div>
              ...
              <div className="mb-6 border border-primary-200 bg-primary-50/40 rounded-2xl p-5">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 mb-4">
                  Set or expand the maximum student and teacher capacity for this school.
                </p>
