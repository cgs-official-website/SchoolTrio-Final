...
export default function PlanManagement() {
  ...
  const fetchPlans = useCallback(async () => {
    ...
    try {
      ...
      const rawList = res?.data?.plans || (Array.isArray(res?.data) ? res.data : []);
      const sortedPlans = [...rawList].sort((a, b) => {
        const idxA = PLAN_ORDER.indexOf((a.name || a.id || '').toLowerCase());
        const idxB = PLAN_ORDER.indexOf((b.name || b.id || '').toLowerCase());
      ...
      console.error('Error fetching subscription plans via REST:', err);
      setPlans([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEditor = (plan) => {
    setCurrentPlan(JSON.parse(JSON.stringify({
      ...
      modules: plan.modules && typeof plan.modules === 'object' ? plan.modules : {}
    })));
    setIsEditing(true);
  };
  ...
  const handleSavePlan = async () => {
    ...
    try {
      const payload = {
        ...
        userLimit: Math.max(1, Number(currentPlan.userLimit) || 1),
        pricePerUserPerYear: Math.max(0, Number(currentPlan.pricePerUserPerYear) || 0),
        cloudStorageGB: Math.max(0, Number(currentPlan.cloudStorageGB) || 5),
        modules: currentPlan.modules || {},
        isActive: currentPlan.active !== false
      };
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-12 rounded-3xl border border-slate-200 dark:border-slate-700 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-6">No subscription plans were found in the database.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          {plans.map(plan => {
            const isPlanActive = plan.isActive !== undefined ? plan.isActive : (plan.active !== false);
            ...
            return (
              <div key={plan.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
                <div className={`p-6 border-b ${isPopular ? 'bg-primary-50 dark:bg-primary-950/40 border-primary-100 dark:border-primary-900' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-800'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">{plan.name}</h3>
                      <div className="mt-1 flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full ${isPlanActive ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{isPlanActive ? 'Active' : 'Inactive'}</span>
                      </div>
                    </div>
                    {isPopular && (
                  ...
                  <div className="mb-2">
                    <div className="flex items-end gap-1">
                      <span className="text-3xl font-black text-slate-900 dark:text-white">₹{plan.pricePerUserPerYear}</span>
                  ...
                  <div className="text-sm font-medium text-slate-600 dark:text-slate-300 flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div>
                    {plan.cloudStorageGB || 5} GB Cloud Storage
                  </div>
                </div>
                    ...
                    {Object.entries(ALL_MODULES).map(([key, label]) => {
                      ...
                      return (
                        <li key={key} className="flex items-center gap-3">
                          {hasModule ? (
                            <div className="text-emerald-500 bg-emerald-50 dark:bg-emerald-950/40 p-0.5 rounded"><Check size={14} strokeWidth={3} /></div>
                          ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-fade-in-up">
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              <div className="grid grid-cols-2 gap-6 mb-8">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary-500 font-medium text-slate-900 dark:text-white bg-white dark:bg-slate-800"
                  />
                </div>
                ...
                <div className="flex items-end">
                  <label className="flex items-center gap-2 cursor-pointer p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 w-full">
                    ...
                    />
                    <span className="font-semibold text-sm text-slate-700 dark:text-slate-200">Active Status</span>
                  </label>
                </div>
                
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">Price (₹/user/year)</label>
                  <input 
                    type="number" 
                    min="0"
                    value={currentPlan.pricePerUserPerYear || 0} 
                ...
                <div>
                  ...
                  <input 
                    type="number" 
                    min="1"
                    value={currentPlan.userLimit || 1} 
                    onChange={e => setCurrentPlan({...currentPlan, userLimit: parseInt(e.target.value) || 1})}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary-500 font-medium text-slate-900 dark:text-white bg-white dark:bg-slate-800"
                  />
                </div>
                ...
                <div>
                  ...
                  <input 
                    type="number" 
                    min="0"
                    value={currentPlan.cloudStorageGB || 5} 
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {Object.entries(ALL_MODULES).map(([key, label]) => {
                    const isChecked = !!(currentPlan.modules && currentPlan.modules[key]);
                    return (
                      <label key={key} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${isChecked ? 'bg-primary-50 dark:bg-primary-950/40 border-primary-200 dark:border-primary-800' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                        ...
                        />
                        <span className={`font-semibold text-sm ${isChecked ? 'text-primary-900 dark:text-primary-100' : 'text-slate-600 dark:text-slate-300'}`}>{label}</span>
                      </label>
                    );
                  })}
                </div>
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-end gap-3 shrink-0">
              ...
              >
                <Save size={20} /> {saving ? 'Saving...' : 'Save Plan Details'}
              </button>
            </div>
          </div>
        </div>
