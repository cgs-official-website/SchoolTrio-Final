...
export default function LicenseUsage() {
  ...
  const fetchUsage = useCallback(async () => {
    ...
    try {
      ...
      const res = await getLicenseUsage(params);
      const rawLicenses = res?.data?.licenses || (Array.isArray(res?.data) ? res.data : []);
      const pagination = res?.data?.pagination || res?.pagination || {};

      setSchools(rawLicenses);
      setTotalCount(pagination.total ?? rawLicenses.length);
    } catch (err) {
      console.error('Failed to load license usage via REST:', err);
      setSchools([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
  ...
  const handleSaveLimit = async () => {
    ...
    try {
      ...
      const schoolId = selectedSchool.schoolId || selectedSchool.id;
      await updateTenantConfig(schoolId, {
        maxStudents: parsedSeats,
        maxStaff: parsedTeachers
      });
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto flex flex-col min-w-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 shrink-0">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-primary-50 dark:bg-primary-950/40 text-primary-600 flex items-center justify-center shrink-0">
            <LuKey size={24} />
          </div>
          <div>
            ...
            <table className="w-full text-left border-collapse min-w-max">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {schools.length === 0 ? (
                  ...
                  schools.map((school) => {
                    const displayName = school.schoolName || school.name || 'Unnamed School';
                    const planName = school.planName || school.plan || 'Standard';
                    const currentStudents = school.students?.current || 0;
                    const studentLimit = school.students?.limit || 500;
                    const currentTeachers = school.teachers?.current || 0;
                    const teacherLimit = school.teachers?.limit || 50;

                    return (
                      <tr key={school.schoolId || school.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        <td className="p-4 font-bold text-slate-900 dark:text-white">
                          <div>
                            <span>{displayName}</span>
                            {school.schoolCode && (
                              <span className="block text-xs font-normal text-slate-400 dark:text-slate-300 mt-0.5">Code: {school.schoolCode}</span>
                            )}
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          {getUsageBar(currentStudents, studentLimit)}
                        </td>
                        <td className="p-4">
                          {getUsageBar(currentTeachers, teacherLimit)}
                        </td>
                        ...
                        <td className="p-4 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-primary-50 text-primary-700 border border-primary-200 hover:bg-primary-600 hover:text-white rounded-xl text-xs font-bold transition-all shadow-sm group dark:bg-primary-950/40 dark:text-primary-300 dark:border-primary-800"
                          >
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">
              ...
              of <span className="font-semibold text-slate-900 dark:text-white">{totalCount}</span> schools
            </span>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-fade-in-up flex flex-col">
            ...
            <div className="p-6 space-y-6">
              <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 min-w-0">
                <h3 className="font-bold text-slate-900 dark:text-white text-lg truncate w-full">{selectedSchool.schoolName || selectedSchool.name}</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3 pt-3 border-t border-slate-200 dark:border-slate-700 text-xs">
                  <div className="min-w-0">
                    ...
                    <p className="font-bold text-slate-900 dark:text-white text-sm mt-0.5">{selectedSchool.students?.current || 0} enrolled</p>
                  </div>
                  <div className="min-w-0">
                    <span className="text-slate-500 dark:text-slate-400 font-medium">Current Teachers:</span>
                    <p className="font-bold text-slate-900 dark:text-white text-sm mt-0.5">{selectedSchool.teachers?.current || 0} registered</p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  {[100, 250, 500, 1000].map(inc => (
                    <button
                      ...
                      className="text-xs px-2.5 py-1 rounded-lg font-bold bg-primary-50 text-primary-700 hover:bg-primary-100 dark:bg-primary-950/40 dark:text-primary-300 dark:border-primary-800 border border-primary-200 transition-colors"
                    >
                      +{inc} Seats
                    </button>
                  ))}
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3 shrink-0">
              ...
              >
                {savingLimit ? 'Applying Expansion...' : 'Apply Expansion'}
              </button>
            </div>
          </div>
        </div>
