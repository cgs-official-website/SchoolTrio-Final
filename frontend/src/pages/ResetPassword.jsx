...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setError('A valid reset token is required. Please request a new password reset link.');
      return;
    }
    ...
    try {
      ...
      if (err.status === 401 || err.code === 'INVALID_TOKEN') {
        ...
        setError(err.message || 'Unable to update password. Please try again later.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-sans selection:bg-primary-500 selection:text-white flex flex-col items-center justify-center relative overflow-hidden px-4">
      ...
      >
        <div className="mb-8 flex flex-col items-center">
          ...
          <h2 className="mt-6 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">New Password</h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 text-center">
            {token ? 'Create a strong, secure password for your account.' : 'Password reset link is invalid or incomplete.'}
          </p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200 dark:border-slate-700 shadow-[0_8px_32px_rgba(0,0,0,0.05)] rounded-3xl p-8 sm:p-10">
          {!token ? (
            ...
            <div className="text-center py-4">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Invalid Reset Link</h3>
              <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
                No password reset token was detected. Please check the link from your email or request a new reset link.
              </p>
              <div className="space-y-3">
                ...
                <button 
                  ...
                  className="w-full py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-medium rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                >
                  Return to Login
                </button>
              </div>
            ...
            >
              ...
              <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
                Your password has been successfully updated. All previous sessions have been invalidated for security. Please log in with your new credentials.
              </p>
              <button 
                onClick={() => navigate('/login')}
                className="w-full py-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white font-bold rounded-xl hover:opacity-90 transition-opacity shadow-[0_4px_14px_rgba(229,189,223,0.4)] focus:outline-none focus:ring-2 focus:ring-primary-500"
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                {loading ? (
                  <>
                    <Loader className="animate-spin" size={20} /> Updating Password...
                  </>
                ) : (
                  'Set New Password'
                )}
