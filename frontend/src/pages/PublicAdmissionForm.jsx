...
export default function PublicAdmissionForm() {
  ...
  useEffect(() => {
    const fetchSchoolAndClasses = async () => {
      ...
      try {
        const res = await admissionsApi.getPublicSchoolMeta(schoolId);
        if (res?.data) {
          if (res.data.school) {
            setSchool(res.data.school);
          }
          if (Array.isArray(res.data.classes)) {
            setClasses(sortClassesAscending(res.data.classes));
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await admissionsApi.submitPublicAdmission(schoolId, applicationPayload);
      const createdApp = res?.data || {};

      setSubmittedData({
        id: createdApp.id,
        applicationNumber: createdApp.applicationNumber || applicationNumber,
        ...customDataPayload,
        ...applicationPayload,
        homeAddress: formData.homeAddress.trim(),
        status: createdApp.status || 'Pending',
        submittedAt: createdApp.submittedAt || new Date().toISOString()
      });
