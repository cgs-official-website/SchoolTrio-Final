...
export default function NoticeFeed({ audienceRole: _audienceRole }) {
  ...
  useEffect(() => {
    ...
    const fetchFeedNotices = async () => {
      try {
        const res = await noticesApi.listNotices({ limit: 50 });
        if (isMounted) {
          const raw = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
          setNotices(raw);
        }
      } catch (err) {
        console.error('Failed to load notices in NoticeFeed:', err);
      } finally {
