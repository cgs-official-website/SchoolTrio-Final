...
export default function TopNavbar({ schoolName, schoolLogo, toggleSidebar, navItems = [] }) {
  ...
  const currentUserId = userProfile?.uid || userProfile?.id || userProfile?.userId;

  const handleMarkNoticeAsRead = async (noticeId) => {
    if (!noticeId) return;
    try {
      await noticesApi.markNoticeViewed(noticeId);
    } catch (err) {
      console.error("Error marking notice viewed:", err);
    }
  ...
  const handleMarkPersonalNotificationAsRead = async (notifId) => {
    if (!notifId) return;
    try {
      await notificationsApi.markNotificationRead(notifId);
    } catch (err) {
      console.error("Error marking personal notification read:", err);
    }
  ...
  const handleMarkAllAsRead = async () => {
    ...
    unreadNotices.forEach(n => {
      if (n.id) noticesApi.markNoticeViewed(n.id).catch(() => {});
    });

    // Mark all personal & system notifications as read on server
    notificationsApi.markAllNotificationsRead().catch((err) => {
  ...
  useEffect(() => {
    ...
    // Fetch authorized notices from REST API
    const fetchNotices = async () => {
      try {
        const res = await noticesApi.listNotices({ limit: 20 });
        if (isMounted) {
          const raw = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
          setNotices(raw);
        }
      } catch (err) {
        console.error("Error loading notices in TopNavbar from REST:", err);
      }
    };

    // Fetch authorized personal & system notifications from REST API
  ...
  }, [userProfile?.schoolId, userProfile?.role, userProfile?.id, userProfile?.userId, userProfile?.uid, showNotifications]);

  const allNotifications = [
    ...notifications.map(n => ({
      ...n,
      type: 'personal',
      title: n.title || (n.type === 'attendance_pending' ? 'Attendance Alert' : (n.type ? n.type.replace(/_/g, ' ').toUpperCase() : 'Notification')),
      description: n.description || n.message,
      priority: n.priority || (n.type === 'attendance_pending' ? 'high' : 'normal'),
                  ...
                  <div className="divide-y divide-slate-100">
                    {allNotifications.map((notice) => {
                      const isUnread = notice.type === 'notice' 
                        ? !notice.viewedBy?.some(v => v.uid === currentUserId || v.userId === currentUserId)
                      ...
                      return (
                        <div 
                          ...
                          onClick={() => {
                            ...
                            if (notice.type === 'personal') {
                              handleMarkPersonalNotificationAsRead(notice.id);
                            }
                            if (notice.type === 'chat') {
                              setShowNotifications(false);
