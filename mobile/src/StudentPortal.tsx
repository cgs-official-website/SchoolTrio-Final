...
// Standardized Module Header Card Component (Matching Admin Parity)
    ...
    <View style={styles.headerCardTopRow}>
      ...
      {badgeText ? (
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}>
          {rightAction.icon && <IconComp name={rightAction.icon} size={14} color="#B07FA8" style={{ marginRight: 6 }} />}
          <Text style={styles.headerCardPrimaryBtnText}>{rightAction.label}</Text>
        </TouchableOpacity>
      ) : null}
    </View>
    {subtitle ? (
...
// --- Reusable Professional Pagination Component (Matching Admin Exactly) ---
...
export const StudentPortal: React.FC<StudentPortalProps> = ({
  ...
  // Two-Step Authentication & Verification State
  const [userProfile, setUserProfile] = useState<any>(null);
  const [isDobVerified, setIsDobVerified] = useState<boolean>(false);
  const [isParentAccount, setIsParentAccount] = useState<boolean>(true);
  const [step2Dob, setStep2Dob] = useState<string>('');
  const [step2Error, setStep2Error] = useState<string>('');
  const [isVerifyingStep2, setIsVerifyingStep2] = useState<boolean>(false);
  const [showStep2DatePicker, setShowStep2DatePicker] = useState<boolean>(false);
  ...
  const dynamicAcademicYear = useMemo(() => {
    ...
    return m >= 5 ? `${y}-${(y + 1).toString().slice(-2)}` : `${y - 1}-${y.toString().slice(-2)}`;
  }, []);
  ...
  const [dynamicTimetables, setDynamicTimetables] = useState<Record<string, any>>({});
  const [transportRoutesList, setTransportRoutesList] = useState<any[]>([]);

  // All Modules Tab State (Matching Admin Standard)
  ...
  // Student & Child Profile State (Pure Real-Time Firestore Data)
  ...
  const [selectedTeacher, setSelectedTeacher] = useState<{ id: string; name: string; role: string; avatar: string; color: string } | null>(null);
  ...
  // Resources State (Sync with schools/${schoolId}/resources)
  const [studentResources, setStudentResources] = useState<any[]>([]);
  const [studentResourceSearch, setStudentResourceSearch] = useState<string>('');
  const [studentResourceType, setStudentResourceType] = useState<string>('All');
  const [studentResourceSubject, setStudentResourceSubject] = useState<string>('All');
  ...
  // All Modules List & Transport Eligibility (Pure In-Memory Evaluation)
  const isTransportEligible = useMemo(() => {
    if (!activeStudent) return false;
    return Boolean(
      activeStudent.transportRouteId ||
      activeStudent.busRoute ||
      activeStudent.transportOpted === true ||
      activeStudent.transport === 'Yes' ||
      activeStudent.transport === 'Bus' ||
      activeStudent.isTransportUser === true ||
      activeStudent.assignedRouteId ||
      activeStudent.busNumber
    );
  }, [activeStudent]);

  const allModulesList: { name: AllModuleType; icon: string; color: string; desc: string }[] = useMemo(() => {
    const list: { name: AllModuleType; icon: string; color: string; desc: string }[] = [
      ...
      { name: 'Resources', icon: 'folder-open-outline', color: '#0284C7', desc: 'Study materials, worksheets & syllabus' },
    ];
    if (isTransportEligible) {
      list.push({
        ...
        icon: 'bus-outline',
        color: '#DB2777',
        desc: 'Bus route, timings & driver contact',
      });
    }
    return list;
  }, [isTransportEligible]);
  ...
  const filteredModules = useMemo(() => {
    ...
    if (!q) return allModulesList;
    return allModulesList.filter(
      m => m.name.toLowerCase().includes(q) || m.desc.toLowerCase().includes(q)
    );
  }, [allModulesList, moduleSearchQuery]);

  const totalModulePages = Math.ceil(filteredModules.length / MODULE_PAGE_SIZE);
  ...
  // Handler: Step 2 Two-Step Verification (DOB ONLY)
  const handleVerifyStep2Dob = async () => {
    const dob = step2Dob.trim();
    if (!dob) {
      setStep2Error("Please select your child's Date of Birth.");
      return;
    }
    setStep2Error('');
    ...
    try {
      ...
      if (db) {
        ...
        if (snap && !snap.empty) {
          ...
          snap.forEach((d: any) => allStudents.push({ id: d.id, ...d.data() }));

          // 1. Find all students matching target DOB
          const dobMatches = allStudents.filter(st => {
            const sDob = normalizeDateStr(st.dob || st.dateOfBirth || st.birthDate);
            return sDob === targetNormDob;
          });

          if (dobMatches.length > 0) {
            const userPhone = (userProfile?.parentPhone || '').replace(/[^0-9]/g, '').slice(-10);
            const userEmailClean = (userProfile?.email || userProfile?.parentEmail || userEmail || '').toLowerCase();
            const userAdm = (userProfile?.studentAdmissionNumber || '').trim().toLowerCase();

            const strongMatch = dobMatches.find(st => {
              const stPhone = (st.parentPhone || st.phone || '').replace(/[^0-9]/g, '');
              const stEmail = (st.parentEmail || st.email || '').toLowerCase();
              const stAdm = (st.admissionNumber || st.rollNo || '').toLowerCase();

              const phoneOk = userPhone && stPhone && (stPhone.includes(userPhone) || userPhone.includes(stPhone.slice(-10)));
              const emailOk = userEmailClean && stEmail && (stEmail === userEmailClean);
              const admOk = userAdm && stAdm && (stAdm === userAdm);

              return phoneOk || emailOk || admOk;
            });

            matchedStudent = strongMatch || dobMatches[0];
          }
        }
      }

      if (!matchedStudent) {
        setStep2Error("No student found matching this Date of Birth. Please check and try again.");
        setIsVerifyingStep2(false);
        return;
      }

      // Persist in Firestore users/{currentUserId}
      if (db && currentUserId) {
        try {
          const existingLinked = userProfile?.linkedStudents || [];
          const alreadyInList = existingLinked.some((s: any) => s.studentId === matchedStudent.id);
          const updatedLinked = alreadyInList
            ? existingLinked
            : [
                ...existingLinked,
                {
                  ...
                  name: matchedStudent.name || matchedStudent.firstName || 'Child',
                  dob: matchedStudent.dob || dob,
          ...
          await db.collection('users').doc(currentUserId).set({
            ...
            linkedClassId: matchedStudent.classId || '',
            linkedStudents: updatedLinked,
          }, { merge: true });
        } catch (persErr) {
          console.warn('Could not persist verification in users doc:', persErr);
        }
      }

      setChildren([matchedStudent]);
      setSelectedChildIndex(0);
      setIsDobVerified(true);
      showToast('Child verified and linked successfully!');
    } catch (err: any) {
      console.error('Step 2 verification error:', err);
      setStep2Error('Verification error. Please check your network and try again.');
    } finally {
      setIsVerifyingStep2(false);
    }
  };

  // Handler: Link Another Child with DOB ONLY (Requirement: No admission number)
  const handleLinkChildSubmit = async () => {
    const dob = linkChildDob.trim();
    if (!dob) {
      setLinkChildError("Please select your child's Date of Birth.");
      return;
    }
    setLinkChildError('');
    ...
    try {
      ...
      if (db) {
        ...
        if (snap && !snap.empty) {
          ...
          const currentLinkedIds = (children || []).map(c => c.id);
          const dobMatches = allStudents.filter(st => {
            const sDob = normalizeDateStr(st.dob || st.dateOfBirth || st.birthDate);
            return sDob === targetNormDob && !currentLinkedIds.includes(st.id);
          });
          ...
          if (dobMatches.length > 0) {
            ...
            foundStudent = strongMatch || dobMatches[0];
          }
        }
      }

      if (!foundStudent) {
        setLinkChildError('No unlinked student found matching this Date of Birth.');
      ...
      if (db && currentUserId) {
        try {
          ...
          const prevLinked = uData.linkedStudents || [];
          const updatedLinked = [
            ...prevLinked,
            {
              studentId: foundStudent.id,
              classId: foundStudent.classId || '',
              name: foundStudent.name || foundStudent.firstName || 'Child',
              dob: foundStudent.dob || dob,
              linkedAt: new Date().toISOString(),
            }
          ];
          await userRef.set({ linkedStudents: updatedLinked }, { merge: true });
        } catch (e) {
          console.warn('Could not persist second child in users doc:', e);
        }
      }
      ...
      setLinkChildError('No student found matching this Date of Birth.');
    } finally {
      setIsLinkingChild(false);
    }
  };
  ...
  // Hook 0a: Listen to users/{currentUserId} for Two-Step Auth & Verified status
  useEffect(() => {
    if (!db || !currentUserId) {
    ...
    const unsub = userRef.onSnapshot(
      (snap: any) => {
        if (snap && snap.exists) {
          ...
          const loginPanel = (uData.loginPanel || '').toLowerCase();
          const isParent = role === 'parent' || loginPanel === 'parent' || (!role && !loginPanel);
          setIsParentAccount(isParent);

          // If isDobVerified === true, portal is unlocked
          // If demo account with pre-linked student where isDobVerified !== false, unlocked
          // If student account (not parent), unlocked
          const verified =
            uData.isDobVerified === true ||
            (!isParent && uData.isDobVerified !== false) ||
            (uData.linkedStudentId && uData.isDobVerified !== false && !role);

          setIsDobVerified(Boolean(verified));
        } else {
          setIsParentAccount(true);
          setIsDobVerified(false);
        }
      },
      (err: any) => console.warn('User profile listener error:', err)
    );

    return () => unsub();
  }, [currentUserId]);

  // Hook 0b: Real-time Teachers Listener (for Staff Chat & dynamic teacher data)
  useEffect(() => {
    if (!db) return;
    const tCol = db.collection('schools').doc(schoolId).collection('teachers');
    const unsub = tCol.onSnapshot(
      (snap: any) => {
        ...
        snap.forEach((d: any) => {
          ...
          list.push({
            ...
            role: data.role || data.designation || 'Teacher',
            avatar: (data.name || data.fullName || 'T').charAt(0).toUpperCase(),
            color: '#B07FA8',
            ...data,
          });
        });
      ...
      (err: any) => console.warn('Teachers listener error:', err)
    );
    return () => unsub();
  }, [schoolId]);

  // Hook 1: Fetch Students for this authenticated account (Pure Real-time Firestore)
  useEffect(() => {
    if (!db) {
      setLoadingInitial(false);
      return;
    ...
    const unsub = studentsCol.onSnapshot(
      (snapshot: any) => {
        ...
        // 1. If userProfile has linkedStudents, match by IDs
        const linkedIds = (userProfile?.linkedStudents || []).map((s: any) => s.studentId);
        if (userProfile?.linkedStudentId) {
          linkedIds.push(userProfile.linkedStudentId);
        }

        let matched = allDocs.filter((st: any) => linkedIds.includes(st.id));

        // 2. Fallback: match by email/phone/admissionNumber
        if (matched.length === 0) {
          const userPhone = (userProfile?.parentPhone || '').replace(/[^0-9]/g, '').slice(-10);
          const admClean = (userProfile?.studentAdmissionNumber || '').toLowerCase();

          matched = allDocs.filter((st: any) => {
            const adm = (st.admissionNumber || st.rollNo || '').toLowerCase();
            ...
            const matchEmail = emailClean && (adm === emailClean || pEmail === emailClean || sEmail === emailClean);
            const matchAdm = admClean && (adm === admClean);
            const matchPhone = userPhone && pPhone && (pPhone.includes(userPhone) || userPhone.includes(pPhone.slice(-10)));

            return matchEmail || matchAdm || matchPhone;
          });
        }
    ...
    return () => unsub();
  }, [userEmail, schoolId, userProfile?.linkedStudentId, userProfile?.linkedStudents]);
  ...
  // Hook 11: Fetch Digital Resources (Real-time sync with schools/${schoolId}/resources)
  useEffect(() => {
    if (!db) return;
    const resCol = db.collection('schools').doc(schoolId).collection('resources');
    const unsub = resCol.onSnapshot((snapshot: any) => {
      if (!snapshot) return;
      const list: any[] = [];
      snapshot.forEach((d: any) => {
        list.push({ id: d.id, ...d.data() });
      });
      list.sort((a: any, b: any) => (b.createdAt || b.date > a.createdAt || a.date ? 1 : -1));
      setStudentResources(list);
    }, (err: any) => console.warn('StudentPortal resources listener err:', err));

    return () => unsub();
  }, [schoolId]);

  // Hook 13: Fetch Timetables (Real-time sync with schools/${schoolId}/timetables)
  useEffect(() => {
    if (!db) return;
    const ttCol = db.collection('schools').doc(schoolId).collection('timetables');
    const unsub = ttCol.onSnapshot(
      (snapshot: any) => {
        if (!snapshot) return;
        const map: Record<string, any> = {};
        snapshot.forEach((d: any) => {
          const data = d.data();
          map[d.id] = data.schedule || data;
        });
        setDynamicTimetables(map);
      },
      (err: any) => console.warn('StudentPortal timetables subscribe err:', err)
    );
    return () => unsub();
  }, [schoolId]);

  // Hook 14: Fetch Transport Routes (Real-time sync with schools/${schoolId}/transportRoutes)
  useEffect(() => {
    if (!db) return;
    const trCol = db.collection('schools').doc(schoolId).collection('transportRoutes');
    const unsub = trCol.onSnapshot(
      (snapshot: any) => {
        if (!snapshot) return;
        const list: any[] = [];
        snapshot.forEach((d: any) => {
        ...
        setTransportRoutesList(list);
      },
      (err: any) => console.warn('StudentPortal transportRoutes subscribe err:', err)
    );
    return () => unsub();
  }, [schoolId]);
  ...
  const renderMessages = () => {
    const activeTeachers = teachersList.length > 0
      ? teachersList
      : [
          { id: 't_default', name: classDetails?.teacherName || 'Class Teacher', role: 'Teacher', avatar: 'T', color: '#b07fa8' }
        ];

    const currentTeacher = selectedTeacher || activeTeachers[0];
  ...
  // =========================================================================
  // RENDER: TAB 5 — ALL MODULES (Admin Parity Design: 2-Column Grid, Search, Pagination)
  // =========================================================================
  const renderAllModules = () => (
    <ScrollView contentContainerStyle={styles.tabScrollContentWithFloatingNav} showsVerticalScrollIndicator={false}>
      <ModuleHeaderCard
        ...
        subtitle="Browse and access all specialized student & parent services."
      />

      {/* Module Search Bar (Matching Admin Exactly) */}
      ...
      {/* 2-Column Responsive Grid matching Admin Exactly */}
      {paginatedModules.length > 0 ? (
        <View style={styles.modulesGridContainer}>
          {paginatedModules.map(mod => (
            <TouchableOpacity
              ...
              <Text style={styles.moduleGridTitleText} numberOfLines={2}>
                {mod.name}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.emptyModuleCardContainer}>
          ...
          <Text style={{ fontSize: 14, fontWeight: '700', color: '#334155', marginTop: 10 }}>
            No modules found
          </Text>
          <Text style={{ fontSize: 12, color: '#64748B', marginTop: 4, textAlign: 'center' }}>
            No services match "{moduleSearchQuery}". Tap below to clear search.
          </Text>
          <TouchableOpacity
            style={{ marginTop: 12, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: '#faedf7', borderRadius: 8 }}
            onPress={() => {
              setModuleSearchQuery('');
              setModulePage(1);
            }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: '#B07FA8' }}>Clear Search</Text>
          </TouchableOpacity>
        </View>
      )}
  ...
  // =========================================================================
  // RENDER: MODULE 12 — TRANSPORT (Available strictly to transport students)
  // =========================================================================
  const renderTransportModule = () => {
    const studentName = activeStudent
      ? `${activeStudent.firstName || ''} ${activeStudent.lastName || activeStudent.name || ''}`.trim()
      : 'Student';

    // Match student to assigned route in live transportRoutesList
    const assignedRouteId = activeStudent?.transportRouteId || activeStudent?.assignedRouteId || activeStudent?.routeId || '';
    const assignedRouteName = (activeStudent?.busRoute || activeStudent?.routeName || activeStudent?.routeNumber || '').trim().toLowerCase();

    const matchingRoute = transportRoutesList.find((r: any) => {
      if (assignedRouteId && (r.id === assignedRouteId || r.routeId === assignedRouteId)) return true;
      if (assignedRouteName) {
        const rNum = (r.routeNumber || '').toString().toLowerCase();
        const rNm = (r.routeName || '').toLowerCase();
        if (rNum && assignedRouteName.includes(rNum)) return true;
        if (rNm && (assignedRouteName.includes(rNm) || rNm.includes(assignedRouteName))) return true;
      }
      return false;
    }) || (transportRoutesList.length > 0 ? transportRoutesList[0] : null);

    const routeName = activeStudent?.busRoute || activeStudent?.routeName || (matchingRoute ? (matchingRoute.routeName || `Route ${matchingRoute.routeNumber || ''}`.trim()) : 'Pending Allocation');
    const vehicleNo = activeStudent?.vehicleNumber || activeStudent?.busNumber || activeStudent?.vehicleNo || matchingRoute?.vehicleNumber || matchingRoute?.busNumber || 'Pending Allocation';
    const driverName = activeStudent?.driverName || matchingRoute?.driverName || matchingRoute?.driver || 'School Transport Team';
    const driverPhone = activeStudent?.driverPhone || matchingRoute?.driverPhone || matchingRoute?.contact || matchingRoute?.driverContact || '';
    const pickupStop = activeStudent?.pickupStop || activeStudent?.stopName || (matchingRoute?.stops && matchingRoute.stops.length > 0 ? (typeof matchingRoute.stops[0] === 'string' ? matchingRoute.stops[0] : matchingRoute.stops[0].name || matchingRoute.stops[0].stopName || 'School Campus') : 'Designated Stop');
    const pickupTime = activeStudent?.pickupTime || matchingRoute?.pickupTime || matchingRoute?.startTime || '07:30 AM';
    const dropTime = activeStudent?.dropTime || matchingRoute?.dropTime || matchingRoute?.endTime || '04:00 PM';
    ...
    return (
      <ScrollView contentContainerStyle={styles.tabScrollContentWithFloatingNav} showsVerticalScrollIndicator={false}>
        <ModuleHeaderCard
          ...
          badgeText="Active Bus Service"
          subtitle="Real-time school bus route, driver details, and stop timings for your child."
        />

        {/* Student Assigned Banner */}
        <View style={[styles.cardContainer, { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, marginBottom: 12 }]}>
          <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: '#faedf7', justifyContent: 'center', alignItems: 'center' }}>
            <IconComp name="person" size={22} color="#B07FA8" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 15, fontWeight: '800', color: '#0F172A' }}>{studentName}</Text>
            <Text style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>
              Class: {activeStudent?.grade || activeStudent?.class || 'Standard'} • Adm: {activeStudent?.admissionNumber || activeStudent?.admissionNo || 'N/A'}
            </Text>
          </View>
          <View style={{ backgroundColor: '#DCFCE7', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 }}>
            <Text style={{ fontSize: 11, fontWeight: '800', color: '#15803D' }}>Opted</Text>
          </View>
        </View>
        ...
        <View style={styles.cardContainer}>
          <View style={styles.cardHeaderRow}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <IconComp name="navigate-outline" size={18} color="#B07FA8" />
              <Text style={styles.cardTitle}>Route & Vehicle Details</Text>
            </View>
            <View style={{ backgroundColor: '#F1F5F9', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 }}>
              <Text style={{ fontSize: 11, fontWeight: '700', color: '#475569' }}>Live</Text>
            </View>
          </View>
          ...
          <View style={{ gap: 10, marginTop: 4 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' }}>
              <Text style={{ fontSize: 13, color: '#64748B' }}>Assigned Route</Text>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#0F172A' }}>{routeName}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' }}>
              <Text style={{ fontSize: 13, color: '#64748B' }}>Vehicle Number</Text>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#0F172A' }}>{vehicleNo}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' }}>
              <Text style={{ fontSize: 13, color: '#64748B' }}>Morning Pickup</Text>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#059669' }}>{pickupTime}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' }}>
              <Text style={{ fontSize: 13, color: '#64748B' }}>Evening Drop-off</Text>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#059669' }}>{dropTime}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6 }}>
              <Text style={{ fontSize: 13, color: '#64748B' }}>Designated Stop</Text>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#0F172A', maxWidth: '60%', textAlign: 'right' }}>{pickupStop}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.cardContainer}>
          <View style={styles.cardHeaderRow}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <IconComp name="call-outline" size={18} color="#B07FA8" />
              <Text style={styles.cardTitle}>Driver & Contact Information</Text>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: '#E2E8F0', justifyContent: 'center', alignItems: 'center' }}>
                <IconComp name="person-outline" size={20} color="#475569" />
              </View>
              <View>
                <Text style={{ fontSize: 14, fontWeight: '700', color: '#0F172A' }}>{driverName}</Text>
                <Text style={{ fontSize: 12, color: '#64748B' }}>{driverPhone || 'Contact School Admin'}</Text>
              </View>
            </View>
            {driverPhone ? (
              <TouchableOpacity
                ...
                onPress={() => Linking.openURL(`tel:${driverPhone.replace(/[^0-9+]/g, '')}`)}
                activeOpacity={0.8}>
                <IconComp name="call" size={14} color="#FFFFFF" />
                <Text style={{ fontSize: 12, fontWeight: '700', color: '#FFFFFF' }}>Call</Text>
              </TouchableOpacity>
            ) : (
              <View style={{ backgroundColor: '#F1F5F9', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 }}>
                <Text style={{ fontSize: 11, fontWeight: '600', color: '#64748B' }}>Contact School</Text>
              </View>
            )}
          </View>
        </View>

        {/* Transport Support Info */}
        <View style={{ backgroundColor: '#F8FAFC', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#E2E8F0' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            <IconComp name="information-circle-outline" size={16} color="#B07FA8" />
            <Text style={{ fontSize: 13, fontWeight: '700', color: '#0F172A' }}>Transport Helpdesk</Text>
          </View>
          <Text style={{ fontSize: 12, color: '#64748B', lineHeight: 17 }}>
            For route adjustments, emergency updates, or leave notifications, please contact the school transport coordinator directly or notify via school messages.
          </Text>
        </View>
      </ScrollView>
    );
      ...
      <ScrollView contentContainerStyle={styles.tabScrollContentWithFloatingNav} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginBottom: 28 }}>
          ...
          {effectiveChildren.length === 0 ? (
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 18, padding: 24, alignItems: 'center', borderWidth: 1, borderColor: '#E2E8F0' }}>
              <IconComp name="people-outline" size={40} color="#CBD5E1" />
              <Text style={{ fontSize: 15, fontWeight: '700', color: '#64748B', marginTop: 10 }}>No Children Linked Yet</Text>
              <Text style={{ fontSize: 12, color: '#94A3B8', textAlign: 'center', marginTop: 4 }}>
                Tap "+ Link / Add Child" above to link your student using their Date of Birth.
              </Text>
            </View>
          ) : (
            <View style={{ gap: 16 }}>
              {effectiveChildren.map((child, idx) => {
                ...
                const fullName = `${child.firstName || ''} ${child.lastName || child.name || 'Student'}`.trim();
                const initials = (child.firstName || child.name || 'ST').substring(0, 2).toUpperCase();
                const className = child.gradeClass || child.className || classDetails?.name || 'Class N/A';
                const admNumber = child.admissionNumber || child.rollNo || child.id || 'N/A';
                const rollNumber = child.rollNumber || child.rollNo || 'N/A';
                const dobFormatted = child.dob || child.dateOfBirth || child.birthDate ? formatDisplayDob(child.dob || child.dateOfBirth || child.birthDate) : 'N/A';
                const genderBlood = `${child.gender || 'Not specified'}${child.bloodGroup ? ` • ${child.bloodGroup}` : ''}`;
            ...
            {weeks.map((week, wIdx) => (
              <View key={`week-${wIdx}`} style={styles.calWeekRow}>
                {week.map((cell: any, cIdx: number) => {
                  const dayEvents = cell.dateStr ? calendarEvents.filter((ev: any) => {
                    const evStart = (ev.startDate || ev.start || '').split('T')[0];
                    const evEnd = (ev.endDate || ev.end || evStart).split('T')[0];
                    const cellDate = cell.dateStr;
                    return (evStart && cellDate >= evStart && cellDate <= evEnd) ||
                           (ev.customDates && ev.customDates.some((cd: string) => cd.includes(cellDate))) ||
                           (ev.start && ev.start.includes(cellDate)) ||
                           (ev.startDate && ev.startDate.includes(cellDate));
                  }) : [];
                  return (
                    <View
                      key={`cell-${wIdx}-${cIdx}`}
  ...
  // =========================================================================
  // RENDER: MODULE 10 — TIMETABLE (Real-time Firestore Sync)
  // =========================================================================
  const renderTimetableModule = () => {
    ...
    // Resolve timetable for activeStudent
    const classId = activeStudent?.classId || '';
    const gradeClass = (activeStudent?.gradeClass || activeStudent?.className || '').trim().toLowerCase();

    // Check direct classId match, or class name match, or any key containing grade
    let studentClassSchedule: any = null;
    if (classId && dynamicTimetables[classId]) {
      studentClassSchedule = dynamicTimetables[classId];
    } else {
      const allKeys = Object.keys(dynamicTimetables);
      const matchedKey = allKeys.find(k => {
        const lower = k.toLowerCase().replace(/[\s\-_]/g, '');
        const targetClean = gradeClass.replace(/[\s\-_]/g, '');
        return targetClean && (lower.includes(targetClean) || targetClean.includes(lower));
      });
      if (matchedKey) {
        studentClassSchedule = dynamicTimetables[matchedKey];
      } else if (allKeys.length === 1) {
        studentClassSchedule = dynamicTimetables[allKeys[0]];
      }
    }

    const rawDaySlots = (studentClassSchedule && studentClassSchedule[selectedTimetableDay]) || [];
    const currentSchedule: Array<{ period: string; time: string; subject: string; teacher: string; room: string }> = Array.isArray(rawDaySlots)
      ? rawDaySlots.map((p: any, idx: number) => ({
          period: p.period || p.periodName || `Period ${idx + 1}`,
          time: p.time || ((p.startTime && p.endTime) ? `${formatTimeStr(p.startTime)} - ${formatTimeStr(p.endTime)}` : p.startTime ? formatTimeStr(p.startTime) : 'Time N/A'),
          subject: p.subject || p.subjectName || 'General',
          teacher: p.teacher || p.teacherName || 'Staff',
          room: p.room || p.roomNumber || 'Room N/A',
        }))
      : [];
    ...
    return (
      <ScrollView contentContainerStyle={styles.tabScrollContentWithFloatingNav} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.screenHeaderRow}>
          <View>
            ...
            <Text style={{ color: '#64748B', fontSize: 12 }}>
              {activeStudent?.gradeClass || 'Student Class'} • Academic Year {dynamicAcademicYear}
            </Text>
          </View>
        </View>
        ...
        {/* Period List Cards or Empty State */}
        {currentSchedule.length === 0 ? (
          <View style={[styles.cardContainer, { alignItems: 'center', padding: 32, marginTop: 10 }]}>
            <View style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: '#F1F5F9', justifyContent: 'center', alignItems: 'center', marginBottom: 12 }}>
              <IconComp name="calendar-outline" size={28} color="#94A3B8" />
            </View>
            <Text style={{ fontSize: 15, fontWeight: '800', color: '#1E293B', marginBottom: 4 }}>
              No Schedule for {selectedTimetableDay}
            </Text>
            <Text style={{ fontSize: 12, color: '#64748B', textAlign: 'center', maxWidth: 260, lineHeight: 18 }}>
              No periods or classes have been published for this class on {selectedTimetableDay}.
            </Text>
          </View>
        ) : (
          <View style={{ gap: 10 }}>
  ...
  // =========================================================================
  // RENDER: MODULE 11 — RESOURCES (Real-time Sync with Teachers' Uploads)
  // =========================================================================
  const renderResourcesModule = () => {
    const studentClassName = (activeStudent?.className || activeStudent?.gradeClass || '').toLowerCase().trim();

    const filteredResources = studentResources.filter((item: any) => {
      if (studentResourceType !== 'All') {
        const typeStr = (item.type || '').toLowerCase();
        if (!typeStr.includes(studentResourceType.toLowerCase())) return false;
      }
      if (studentResourceSubject !== 'All') {
        if (item.subject !== studentResourceSubject) return false;
      }
      if (studentResourceSearch.trim()) {
        const q = studentResourceSearch.toLowerCase();
    ...
    const resourceTypes = ['All', 'Document', 'Video', 'Link', 'Image'];
    ...
    return (
      <ScrollView contentContainerStyle={styles.tabScrollContentWithFloatingNav} showsVerticalScrollIndicator={false}>
        <ModuleHeaderCard
          ...
          subtitle="Access syllabus, worksheets, reference links & class notes uploaded by your teachers."
          badgeText={`${filteredResources.length} Files`}
        />
        ...
        <SearchInputBox
          ...
          placeholder="Search materials by title, subject, teacher..."
          value={studentResourceSearch}
          onChangeText={setStudentResourceSearch}
          onClear={() => setStudentResourceSearch('')}
        />
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {resourceTypes.map(t => {
              const active = studentResourceType === t;
              return (
                <TouchableOpacity
                  key={t}
                  onPress={() => setStudentResourceType(t)}
                  style={{
                    paddingHorizontal: 14,
                    paddingVertical: 7,
                    borderRadius: 20,
                  ...
                  <Text style={{ fontSize: 12, fontWeight: '700', color: active ? '#FFFFFF' : '#64748B' }}>
                    {t}
                  </Text>
                </TouchableOpacity>
              );
        ...
        {filteredResources.length === 0 ? (
          <View style={{ alignItems: 'center', justifyContent: 'center', paddingVertical: 48, backgroundColor: '#FFFFFF', borderRadius: 20, borderWidth: 1, borderColor: '#E2E8F0' }}>
            <IconComp name="folder-open-outline" size={48} color="#CBD5E1" />
            <Text style={{ fontSize: 15, fontWeight: '700', color: '#64748B', marginTop: 12 }}>No Resources Found</Text>
            <Text style={{ fontSize: 12, color: '#94A3B8', marginTop: 4, textAlign: 'center', paddingHorizontal: 24 }}>
              Materials uploaded by teachers will appear here automatically in real time.
            </Text>
          </View>
        ) : (
          <View style={{ gap: 12 }}>
            {filteredResources.map((res: any) => {
              const typeIcon =
                res.type === 'video' ? 'videocam-outline' :
                res.type === 'image' ? 'image-outline' :
                res.type === 'link' ? 'link-outline' : 'document-text-outline';
              const typeColor =
                res.type === 'video' ? '#EF4444' :
                res.type === 'image' ? '#10B981' :
                res.type === 'link' ? '#8B5CF6' : '#0284C7';
                ...
                <View
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12 }}>
                    <View
                      ...
                      <IconComp name={typeIcon} size={22} color={typeColor} />
                    </View>
                    <View style={{ flex: 1 }}>
                      ...
                      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6, alignItems: 'center' }}>
                        {!!res.subject && (
                          <View style={{ backgroundColor: '#EFF6FF', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 }}>
                            <Text style={{ fontSize: 11, fontWeight: '700', color: '#2563EB' }}>{res.subject}</Text>
                          </View>
                        )}
                        {!!res.class && (
                          <View style={{ backgroundColor: '#F8FAFC', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8, borderWidth: 1, borderColor: '#E2E8F0' }}>
                            <Text style={{ fontSize: 11, fontWeight: '600', color: '#64748B' }}>{res.class}</Text>
                          </View>
                        )}
                        {!!res.size && (
                          <Text style={{ fontSize: 11, color: '#94A3B8' }}>• {res.size}</Text>
                        )}
                      </View>
                    </View>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    <View>
                      <Text style={{ fontSize: 11, color: '#94A3B8' }}>
                        Shared by {res.teacherName || 'Teacher'}
                      </Text>
                      <Text style={{ fontSize: 10, color: '#CBD5E1', marginTop: 1 }}>
                        {res.date || res.createdAt?.split('T')[0] || 'Recently'}
                      </Text>
                    </View>
                    <TouchableOpacity
                      onPress={() => {
                        if (res.url) {
                          Linking.openURL(res.url).catch(() => {
                            Alert.alert('Unable to open', 'Please verify the URL: ' + res.url);
                          });
                        } else {
                          Alert.alert('Notice', 'No URL or attachment attached to this resource.');
                        }
                      }}
                      style={{
                        flexDirection: 'row',
                      ...
                      activeOpacity={0.8}>
                      <IconComp name="cloud-download-outline" size={15} color="#FFFFFF" />
    ...
    if (activeAllModule) {
      return (
        <View style={{ flex: 1 }}>
          ...
          {activeAllModule === 'Resources' && renderResourcesModule()}
          {activeAllModule === 'Transport' && renderTransportModule()}
        </View>
      );
    }
  ...
  // =========================================================================
  // STEP 2: TWO-STEP VERIFICATION LOCK SCREEN (DOB ONLY)
  // =========================================================================
  const renderParentDobLockScreen = () => {
    return (
      <View style={styles.container}>
        ...
        <View style={styles.topHeaderBar}>
          ...
          <View style={styles.schoolBrandDetails}>
            ...
            <View style={styles.schoolSubRow}>
              <View style={[styles.officialBadgeInline, { backgroundColor: '#F3E8FF', borderColor: '#D8B4FE' }]}>
                <IconComp name="lock-closed" size={10} color="#7E22CE" />
                <Text style={[styles.officialBadgeText, { color: '#7E22CE' }]}>Step 2: Verification</Text>
              </View>
            </View>
          </View>
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#F1F5F9', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 }}
            onPress={onLogout}>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 20, justifyContent: 'center' }} showsVerticalScrollIndicator={false}>
          <View style={{ backgroundColor: '#FFFFFF', borderRadius: 24, padding: 24, borderWidth: 1, borderColor: '#E2E8F0', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.05, shadowRadius: 12, elevation: 3 }}>
            {/* Lock Badge */}
            <View style={{ width: 64, height: 64, borderRadius: 32, backgroundColor: '#FAF5FF', borderWidth: 2, borderColor: '#E9D5FF', justifyContent: 'center', alignItems: 'center', alignSelf: 'center', marginBottom: 16 }}>
              <IconComp name="shield-checkmark" size={32} color="#9333EA" />
            </View>

            {/* Title & Description */}
            <Text style={{ fontSize: 22, fontWeight: '800', color: '#0F172A', textAlign: 'center', letterSpacing: -0.5 }}>
              Two-Step Verification
            </Text>
            <Text style={{ fontSize: 13, color: '#64748B', textAlign: 'center', marginTop: 8, lineHeight: 19 }}>
              To securely connect your parent account and unlock your child's academic records, please verify your child's Date of Birth.
            </Text>

            {/* Error Message Banner */}
            {step2Error ? (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#FEF2F2', borderWidth: 1, borderColor: '#FECACA', borderRadius: 12, padding: 12, marginTop: 16 }}>
                <IconComp name="alert-circle" size={18} color="#EF4444" />
                <Text style={{ flex: 1, fontSize: 12, fontWeight: '600', color: '#B91C1C', lineHeight: 16 }}>
                  {step2Error}
                </Text>
              </View>
            ) : null}

            {/* Date of Birth Picker Field (DOB ONLY) */}
            <Text style={{ fontSize: 12, fontWeight: '800', color: '#334155', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 20, marginBottom: 8 }}>
              Child's Date of Birth <Text style={{ color: '#EF4444' }}>*</Text>
            </Text>
            ...
            <TouchableOpacity
              style={{
                ...
                backgroundColor: '#F8FAFC',
                borderWidth: 1.5,
                borderColor: step2Dob ? '#B07FA8' : '#CBD5E1',
                borderRadius: 14,
                paddingHorizontal: 16,
                paddingVertical: 14,
              }}
              onPress={() => setShowStep2DatePicker(true)}
              activeOpacity={0.8}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                <IconComp name="calendar-outline" size={20} color={step2Dob ? '#B07FA8' : '#94A3B8'} />
                <Text style={{ fontSize: 15, fontWeight: '700', color: step2Dob ? '#0F172A' : '#94A3B8' }}>
                  {step2Dob ? formatDisplayDob(step2Dob) : 'Select Date of Birth'}
                </Text>
              </View>
              <View style={{ backgroundColor: '#EDE9FE', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 }}>
                <Text style={{ fontSize: 11, fontWeight: '700', color: '#7E22CE' }}>Pick Date</Text>
              </View>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={{
                ...
                shadowRadius: 8,
                elevation: 4,
                opacity: isVerifyingStep2 ? 0.7 : 1,
              }}
              onPress={handleVerifyStep2Dob}
              disabled={isVerifyingStep2}
              activeOpacity={0.85}>
              {isVerifyingStep2 ? (
                <ActivityIndicator size="small" color="#FFFFFF" />
              ) : (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <IconComp name="lock-open-outline" size={18} color="#FFFFFF" />
                  <Text style={{ fontSize: 15, fontWeight: '800', color: '#FFFFFF' }}>
                    Verify & Connect Child
                  </Text>
                </View>
              )}
            </TouchableOpacity>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 18 }}>
              <IconComp name="shield-outline" size={14} color="#94A3B8" />
              <Text style={{ fontSize: 11, color: '#94A3B8', fontWeight: '500' }}>
                End-to-End Encrypted Verification • Instant Access
              </Text>
            </View>
          </View>
        </ScrollView>
        ...
        <CalendarDatePickerModal
          visible={showStep2DatePicker}
          title="Select Child's Date of Birth"
          currentDateStr={step2Dob}
          onSelectDate={dStr => {
            setStep2Dob(dStr);
            setStep2Error('');
            setShowStep2DatePicker(false);
          }}
          onClose={() => setShowStep2DatePicker(false)}
        />
      </View>
    );
  };
  ...
  // Two-Step Verification Gate: Unverified Parent Accounts See Step 2 Lock Screen
  if (isParentAccount && !isDobVerified) {
    return renderParentDobLockScreen();
  }

  // =========================================================================
  // MAIN RETURN (VERIFIED ACCOUNT)
  // =========================================================================
  return (
    <View style={styles.container}>
      {/* Top Header Bar */}
      ...
      <Modal
        ...
        <KeyboardAvoidingView
          ...
          <View style={[styles.modalCardContainer, { maxWidth: 380, width: '92%' }]}>
            ...
            {/* Field: Date of Birth (DOB Only) */}
            <Text style={styles.linkFieldLabel}>
              Child's Date of Birth <Text style={{ color: '#EF4444' }}>*</Text>
            </Text>
            <TouchableOpacity
              style={styles.linkDobInputRow}
              onPress={() => setShowLinkDatePicker(true)}
              activeOpacity={0.85}>
              <Text style={{ flex: 1, fontSize: 14, color: linkChildDob ? '#0F172A' : '#94A3B8', fontWeight: linkChildDob ? '600' : '400', paddingVertical: 10, paddingHorizontal: 12 }}>
                {linkChildDob ? formatDisplayDob(linkChildDob) : 'Select child Date of Birth'}
              </Text>
              <View style={styles.linkDobCalendarIconBtn}>
            ...
            <KeyboardAwareFormScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 10 }}>
              ...
              {addOrLinkTab === 'link_enrolled' ? (
                <View>
                  ...
                  <Text style={[styles.modalFieldLabel, { marginTop: 8 }]}>
                    CHILD'S DATE OF BIRTH <Text style={{ color: '#EF4444' }}>*</Text>
                  </Text>
                  <TouchableOpacity
                    style={styles.modalDobInputBox}
                    onPress={() => setShowLinkDatePicker(true)}
                    activeOpacity={0.85}>
                    <Text style={{ flex: 1, fontSize: 14, color: linkChildDob ? '#0F172A' : '#94A3B8', fontWeight: linkChildDob ? '600' : '400', paddingVertical: 10, paddingHorizontal: 12 }}>
                      {linkChildDob ? formatDisplayDob(linkChildDob) : 'Select child Date of Birth'}
...
const styles = StyleSheet.create({
  ...
  // All Modules Screen & Standardized Admin Styles
  screenHeaderRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  // Module Hero Header Card (Matching Admin Parity Exactly)
  ...
  // 2-Column Module Grid (Matching Admin Exactly)
  ...
  // Pagination (Matching Admin Exactly)
  paginationWrapper: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
