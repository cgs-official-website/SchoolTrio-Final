...
export const TransportManagementModule: React.FC<TransportManagementProps> = ({
  ...
  // Filtered Student Assignments (Matching Image 5 & Allocated Class)
  const filteredStudents = useMemo(() => {
    const seen = new Set<string>();
    return students.filter(s => {
      // Deduplicate by ID or admission number
      const dedupeKey = (s.id || s.admissionNumber || s.name || '').toLowerCase().trim();
      if (seen.has(dedupeKey)) return false;
      seen.add(dedupeKey);

      // Class Filter: Filter for teacher's allocated class (e.g., 8th-A)
      if (classFilter && classFilter !== 'all') {
        const cf = classFilter.toLowerCase().replace(/[\s\-_]/g, '');
        const sg = (s.grade || '').toLowerCase().replace(/[\s\-_]/g, '');
        const sc = (s.class || '').toLowerCase().replace(/[\s\-_]/g, '');
        const matchesClass =
          sg.includes(cf) ||
          sc.includes(cf) ||
          cf.includes(sg) ||
          cf.includes(sc) ||
          ((cf.includes('8') || cf.includes('grade8')) && (sg.includes('8') || sc.includes('8')));
  ...
  }, [students, routes, assignmentSearchQuery, assignmentFilterTab, classFilter]);
